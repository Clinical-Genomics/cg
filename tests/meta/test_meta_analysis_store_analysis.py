...
def create_config_stream(pipeline):
    return 'dummy_config_stream'


def test_store_analysis_different_pipeline_creates_new_analysis(analysis_store: Store,
                                                                analysis_api: AnalysisAPI):
    """Store analysis should creates a new analysis if the pipeline of the started one is
    different"""

    # GIVEN a status db with a family with a started analysis with a pipeline
    analysis = add_analysis(analysis_store, started=True, family_id='family')
    sample = add_sample(analysis_store, data_analysis='pip1')
    analysis_store.relate_sample(analysis.family, sample=sample, status='unknown')
    assert analysis.started_at
    assert not analysis.completed_at
    nr_analyses_before_store_analysis = len(analysis.family.analyses)

    # WHEN storing the complete analysis for another pipeline
    config_stream = create_config_stream('pip2')
    analysis_api.store_analysis(config_stream)

    # THEN we should have a new analysis object that is marked as completed
    assert nr_analyses_before_store_analysis + 1 == len(analysis.family.analyses)
    assert analysis.started_at
...
def test_store_completed_at_to_existing_analysis(analysis_store: Store, analysis_api: AnalysisAPI):
    ...
    assert nr_analyses_before_store_analysis == len(analysis.family.analyses)
