...
@pytest.mark.parametrize(
    "upload_context", ["mip-rna", "tomte"], indirect=True
)
def test_upload_rna_alignment_file_to_scout(
    ...
    upload_context.status_db = rna_store

    # GIVEN an RNA case with an alignment file stored in HK

    # WHEN running the method to upload RNA files to Scout
    upload_context.scout_api.upload_rna_alignment_file(case_id=rna_case_id, dry_run=True)
...
def test_upload_rna_junctions_to_scout(
    ...
    # WHEN running the method to upload RNA files to Scout
    caplog.set_level(logging.INFO)
    upload_context.scout_api.upload_rna_junctions_to_scout(case_id=rna_case_id, dry_run=True)
...
#     dna_cases: list[str] = upload_context._dna_cases_related_to_dna_sample(
#         dna_sample=dna_sample,
#         collaborators=rna_sample.customer.collaborators,
#     )
