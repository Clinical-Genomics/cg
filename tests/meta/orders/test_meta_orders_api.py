...
@patch(
    "cg.meta.orders.ticket_handler.FormDataRequest.submit",
    return_value=None,
)
...
def test_submit(
    ...
    order_data = OrderIn.parse_obj(
        obj=all_orders_to_submit[order_type],
        project=order_type,
    )
...
def test_submit_scout_legal_sample_customer(
    ...
    # that the samples originate from
    collaboration = sample_store.add_collaboration(
        "customer999only",
        "customer 999 only group",
    )
...
def test_submit_duplicate_sample_case_name(
    ...
    for sample in order_data.samples:
        ...
        assert store.get_case_by_name_and_customer(
            customer=customer,
            case_name=case_id,
        )
