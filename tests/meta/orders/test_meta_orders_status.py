...
def test_store_rml(orders_api, base_store, rml_status_data, ticket: str):
    ...
    assert base_store.get_pools_for_customer_and_enquiry().count() == 0
    ...
    assert base_store.get_pools_for_customer_and_enquiry().count() == base_store.families().count()
    ...
    new_pool = base_store.get_pools_for_customer_and_enquiry().first()
