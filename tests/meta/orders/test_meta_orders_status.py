...
def test_store_rml(orders_api, base_store, rml_status_data, ticket_id: str):
    ...
    assert base_store.get_filtered_cases().count() == 0
    ...
    assert base_store._get_query(table=Pool).count() == base_store.get_filtered_cases().count()
    ...
    new_case = base_store.get_filtered_cases().first()
...
def test_store_samples(orders_api, base_store, fastq_status_data, ticket_id: str):
    ...
    assert base_store.get_filtered_cases().count() == 2
    first_sample = new_samples[0]
    assert len(first_sample.links) == 2
    family_link = first_sample.links[0]
    assert family_link.family in base_store.get_filtered_cases()
...
def test_store_microbial_case_data_analysis_stored(
    ...
    microbial_case = base_store.get_filtered_cases().first()
...
def test_store_mip(orders_api, base_store, mip_status_data, ticket_id: str):
    ...
    assert not base_store.get_samples()
    assert base_store.get_filtered_cases().first() is None
...
def test_store_existing_case(
    ...
    base_store.close()
    new_cases = base_store.get_filtered_cases().all()
    ...
    base_store.close()
    rerun_cases = base_store.get_filtered_cases().all()
