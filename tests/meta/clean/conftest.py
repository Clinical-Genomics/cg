...
from tests.apps.demultiplex.conftest import fixture_demux_run_dir, fixture_novaseq_dir
...
def fixture_correct_flow_cell_path(
    demultiplexed_flowcells_working_directory, correct_flow_cell
) -> Path:
    """Full path to a correctly named flow cell directory in demultiplexed-runs"""
    return Path(demultiplexed_flowcells_working_directory, correct_flow_cell)


@pytest.fixture(name="incorrect_flow_cell_path_too_long")
def fixture_incorrect_flow_cell_path_too_long(
    demultiplexed_flowcells_working_directory, incorrect_flow_cell_too_long
) -> Path:
    """Full path to an incorrectly named flow cell directory in demultiplexed-runs"""
    return Path(demultiplexed_flowcells_working_directory, incorrect_flow_cell_too_long)


@pytest.fixture(name="incorrect_flow_cell_path_extension")
def fixture_incorrect_flow_cell_path_extension(
    demultiplexed_flowcells_working_directory, incorrect_flow_cell_extension
) -> Path:
    """Full path to an incorrectly named flow cell directory in demultiplexed-runs"""
    return Path(demultiplexed_flowcells_working_directory, incorrect_flow_cell_extension)
