...
def fixture_correct_flow_cell_path(
    demultiplexed_flow_cells_directory, correct_flow_cell_name
) -> Path:
    """Full path to a correctly named flow cell directory in demultiplexed-runs."""
    return Path(demultiplexed_flow_cells_directory, correct_flow_cell_name)


@pytest.fixture(name="incorrect_flow_cell_path_too_long")
def fixture_incorrect_flow_cell_path_too_long(
    demultiplexed_flow_cells_directory, incorrect_flow_cell_too_long
) -> Path:
    """Full path to an incorrectly named flow cell directory in demultiplexed-runs."""
    return Path(demultiplexed_flow_cells_directory, incorrect_flow_cell_too_long)


@pytest.fixture(name="incorrect_flow_cell_path_extension")
def fixture_incorrect_flow_cell_path_extension(
    demultiplexed_flow_cells_directory, incorrect_flow_cell_extension
) -> Path:
    """Full path to an incorrectly named flow cell directory in demultiplexed-runs."""
    return Path(demultiplexed_flow_cells_directory, incorrect_flow_cell_extension)


@pytest.fixture(name="non-existent_flow_cell_path")
def fixture_non_existent_flow_cell_path(
    demultiplexed_flow_cells_directory, incorrect_flow_cell_name
) -> Path:
    """Full path to an incorrectly named flow cell directory in demultiplexed-runs."""
    return Path(demultiplexed_flow_cells_directory, incorrect_flow_cell_name)
