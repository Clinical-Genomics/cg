...
def test_transfer_payload_model_dump(
    ...
    assert isinstance(
        dict_representation.get("metadataList", None),
        list,
    )
...
def test_transfer_payload_correct_source_root(
    transfer_payload: TransferPayload,
):
...
def test_transfer_payload_correct_destination_root(
    transfer_payload: TransferPayload,
):
...
def test_auth_header_old_token(
...
def test_auth_header_new_token(
    ddn_dataflow_client: DDNDataFlowClient,
):
...
def test_archive_file(
    ...
    mock_request_submitter.assert_called_once_with(
        ...
        url=urljoin(
            base=ddn_dataflow_client.url,
            url=DataflowEndpoints.ARCHIVE_FILES,
        ),
        headers=dict(
            ddn_dataflow_client.headers,
            **ddn_dataflow_client.auth_header,
        ),
        json={
            "pathInfo": [
                {
...
def test_retrieve_files(
    ...
    mock_request_submitter.assert_called_once_with(
        ...
        url=urljoin(
            base=ddn_dataflow_client.url,
            url=DataflowEndpoints.RETRIEVE_FILES,
        ),
        headers=dict(
            ddn_dataflow_client.headers,
            **ddn_dataflow_client.auth_header,
...
def test_create_transfer_request_retrieve(
    ddn_dataflow_client: DDNDataFlowClient,
    miria_file_retrieve: MiriaObject,
):
