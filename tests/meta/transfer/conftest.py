...
def transfer_lims_api(
    sample_store: Store,
) -> Generator[TransferLims, None, None]:
    ...
    for sample in samples:
        ...
        for read in [1, 2]:
            Path(
                ticket_folder,
                sample,
                f"{sample}_fastq_{read}.fastq.gz",
            ).touch(exist_ok=True)
            Path(
                ticket_folder,
                sample,
                f"{sample}_fastq_{read}.fastq.gz.md5",
            ).touch(exist_ok=True)
