...
from tests.meta.demultiplex.conftest import (
    get_all_files_in_directory_tree,
)
...
def test_validate_demux_complete_flow_cell_directory_when_it_exists(
    tmp_path: Path,
):
...
def test_validate_demux_complete_flow_cell_directory_when_it_does_not_exist(
    tmp_path: Path,
):
...
def test_get_sample_sheet_path_found(
    tmp_path: Path,
):
...
def test_get_sample_sheet_path_found_in_nested_directory(
    tmp_path: Path,
):
    # GIVEN a temporary directory as the flow cell directory
    flow_cell_directory: Path = tmp_path
...
def test_get_sample_sheet_path_not_found(
    tmp_path: Path,
):
...
def test_parse_flow_cell_directory_data_valid():
    ...
    result = FlowCellDirectoryData(
        Path(flow_cell_run_directory),
        "dummy_bcl_converter",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Path(
                "flow_cell_dir",
                DemultiplexingDirsAndFiles.INTER_OP,
                "some_file.txt",
            ),
            True,
        ),
...
def test_get_existing_manifest_file_illumina(
    lsyncd_source_directory: Path,
):
    ...
    assert manifest_file == Path(
        lsyncd_source_directory,
        DemultiplexingDirsAndFiles.ILLUMINA_FILE_MANIFEST,
    )


def test_get_existing_manifest_file(
    lsyncd_source_directory: Path,
):
...
def test_get_existing_manifest_file_missing(
    lsyncd_source_directory: Path,
):
...
def test_is_syncing_complete_true(
    lsyncd_source_directory: Path,
    lsyncd_target_directory: Path,
):
    ...
    is_directory_synced: bool = is_syncing_complete(
        source_directory=lsyncd_source_directory,
        target_directory=lsyncd_target_directory,
    )
...
def test_are_all_files_synced_false(
    ...
    Path(
        lsyncd_target_directory,
        files_at_source[0],
    ).unlink()

    # WHEN checking if the syncing is complete
    is_directory_synced: bool = are_all_files_synced(
        files_at_source=files_at_source,
        target_directory=lsyncd_target_directory,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [NANOPORE_SEQUENCING_SUMMARY_PATTERN.replace("*", "FlowCell_ID1_ID2")],
            True,
        ),
    ],
)
def test_is_manifest_file_required(
...
def test_is_manifest_file_required_nanopore_data(
    tmp_path: Path,
):
    """Tests if a manifest file is needed given the files present."""
    # GIVEN a source directory
...
def test_create_manifest_file(
    tmp_flow_cells_directory_ready_for_demultiplexing_bcl_convert: Path,
):
    ...
    assert_file_contains_all_file_paths(
        manifest_file=manifest_file,
        files_in_file=all_files,
    )
...
def test_get_undetermined_fastqs_no_matching_files(
    tmp_path,
):
...
def test_get_undetermined_fastqs_single_matching_file(
    tmp_path,
):
...
def test_get_undetermined_fastqs_multiple_matching_files(
    tmp_path,
):
    ...
    expected_files = [
        Path(
            tmp_path,
            "Undetermined_L001_R1.fastq.gz",
        ),
        Path(
            tmp_path,
            "Undetermined_L001_R2.fastq.gz",
        ),
    ]
...
def test_get_nanopore_summary_file_exists(
    tmp_path: Path,
):
    # GIVEN a directory with a correct file in it
    file_in_directory = Path(
        tmp_path,
        "final_summary_some_flow_cell.txt",
    )
...
def test_get_nanopore_summary_file(
    tmp_path: Path,
):
...
def test_is_nanopore_sequencing_complete(
    tmp_path: Path,
):
    # GIVEN a directory with a correct file in it
    file_in_directory = Path(
...
def test_is_nanopore_sequencing_incomplete(
    tmp_path: Path,
):
