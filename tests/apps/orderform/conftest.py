...
def mip_json_orderform(orderforms: Path) -> dict:
    ...
    return json.load(open(orderforms / "mip.json"))
...
def fastq_orderform(orderforms: Path):
    ...
    _file = orderforms / f"{Orderform.FASTQ}.{ORDERFORM_VERSIONS[Orderform.FASTQ]}.fastq.xlsx"
    return str(_file)
...
def mip_orderform(orderforms: Path) -> str:
    ...
    _file = orderforms / f"{Orderform.MIP_DNA}.{ORDERFORM_VERSIONS[Orderform.MIP_DNA]}.mip.xlsx"
    return str(_file)
...
def mip_rna_orderform(orderforms: Path) -> str:
    ...
    _file = orderforms / f"{Orderform.MIP_RNA}.{ORDERFORM_VERSIONS[Orderform.MIP_RNA]}.mip_rna.xlsx"
    return str(_file)
