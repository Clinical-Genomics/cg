...
def test_create_novaseq_flowcell(stats_api: StatsAPI, bcl2fastq_demux_results: DemuxResults):
    ...
    assert not find.get_flowcell_id(flowcell_name=bcl2fastq_demux_results.flowcell.id)
    ...
    assert find.get_flowcell_id(flowcell_name=bcl2fastq_demux_results.flowcell.id)
