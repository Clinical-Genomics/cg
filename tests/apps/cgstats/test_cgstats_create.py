...
def test_create_support_parameters(stats_api: StatsAPI, demux_results: DemuxResults):
    ...
    support_parameters_id = find.get_support_parameters_id(
...
def test_create_data_source(stats_api: StatsAPI, demux_results: DemuxResults):
    ...
    assert not find.get_datasource_id(manager=stats_api, demux_results=demux_results)
    ...
    assert find.get_datasource_id(manager=stats_api, demux_results=demux_results)
...
def test_create_demux(stats_api: StatsAPI, demux_results: DemuxResults):
    ...
    assert not find.get_demux_id(manager=stats_api, flowcell_object_id=flowcell.flowcell_id)
    ...
    assert find.get_demux_id(manager=stats_api, flowcell_object_id=flowcell.flowcell_id)
