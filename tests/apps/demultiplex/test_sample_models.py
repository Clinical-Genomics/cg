...
def test_validate_inputs_bcl_convert_sample_missing_attribute(
    lims_sample: dict,
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            "CCATTCGANNNNNNNNN-GTTGTCCG",
            "CCATTCGA",
            "GTTGTCCG",
        ),
        (
            "CGATAGCAGG-AATGCTACGA",
            "CGATAGCAGG",
            "AATGCTACGA",
        ),
        ("ACAGCAAG", "ACAGCAAG", ""),
...
def test_separate_indexes_dual_run(
    lims_index: str,
    expected_index_1: str,
    expected_index2: str,
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            NOVASEQ_6000_POST_1_5_KITS_INDEX_SETTINGS,
            f"{INDEX_TWO_PAD_SEQUENCE}GCCAAGGT",
        ),
        (
            NO_REVERSE_COMPLEMENTS_INDEX_SETTINGS,
            f"GCCAAGGT{INDEX_TWO_PAD_SEQUENCE}",
        ),
    ],
    ids=[
        "reverse complement",
...
def test_pad_indexes_needs_padding(
...
@pytest.mark.parametrize(
    ...
    [
        (
            "CGATAGCAGG",
            10,
            IndexOverrideCycles.FULL_10_INDEX,
        ),
        (
            "CCATTCGANNNNNNNNN-GTTGTCCG",
            8,
            IndexOverrideCycles.FULL_8_INDEX,
        ),
        (
            "GTTCCAAT",
            8,
        ...
        (
            "GTTCCAAT",
            10,
            IndexOverrideCycles.INDEX_8_IGNORED_2,
        ),
    ],
    ids=[
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            10,
            None,
            IndexOverrideCycles.IGNORED_10_INDEX,
        ),
        (
            "GTTCCAAT",
            8,
            None,
            IndexOverrideCycles.IGNORED_8_INDEX,
        ),
        (
            "GTTCCAAT-AATTCTGC",
            10,
            True,
            IndexOverrideCycles.INDEX_8_IGNORED_2_REVERSED,
        ),
        (
            "GTTCCAAT-AATTCTGC",
            10,
...
def test_get_index2_override_cycles(
    ...
    index2_cycles: str = sample._get_index2_override_cycles(
        len_index2_cycles=index2_cycles,
...
@pytest.mark.parametrize(
    ...
    [
        ("bcl_convert_samples_similar_index1", 0),
        ("bcl_convert_samples_similar_index2", 1),
    ],
    ids=["barcode1_0", "barcode1_1"],
)
def test_update_barcode_mismatches_1(
    sample_list_fixture: str,
    expected_barcode_mismatch: int,
    request: pytest.FixtureRequest,
):
...
@pytest.mark.parametrize(
    "sample_list_fixture, expected_barcode_mismatch",
    [
        ("bcl_convert_samples_similar_index1", 1),
        ("bcl_convert_samples_similar_index2", 0),
    ],
...
def test_update_barcode_mismatches_2(
    ...
    sample_to_update._update_barcode_mismatches_2(
        samples_to_compare=sample_list,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "novaseq_6000_run_parameters_post_1_5_kits",
            get_reverse_complement_dna_seq("GCCAAGGT"),
        ),
