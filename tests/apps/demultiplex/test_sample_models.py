...
def test_validate_inputs_bcl_convert_sample_missing_attribute(lims_sample: dict):
    ...
    with pytest.raises(AttributeError) as exc_info:
        IlluminaSampleIndexSettings.validate_inputs(lims_sample=lims_sample)
...
def test_separate_indexes_dual_run(lims_index: str, expected_index_1: str, expected_index2: str):
    ...
    sample = IlluminaSampleIndexSettings(lane=1, index=lims_index, sample_id="ACC123")
...
def test_separate_indexes_single_run(
    index1_8_nt_sequence_from_lims: str, bcl_convert_flow_cell_sample: IlluminaSampleIndexSettings
):
...
def test_update_override_cycles(
    bcl_convert_flow_cell_sample: IlluminaSampleIndexSettings,
...
def test_update_barcode_mismatches_1(
    ...
    sample_list: list[IlluminaSampleIndexSettings] = request.getfixturevalue(sample_list_fixture)

    # GIVEN a FlowCellSampleBCLConvert
    sample_to_update: IlluminaSampleIndexSettings = sample_list[0]
...
def test_process_indexes_for_sample_sheet_bcl_convert(
    ...
    run_parameters: RunParameters = request.getfixturevalue(run_parameters_fixture)
    raw_lims_samples: list[IlluminaSampleIndexSettings] = request.getfixturevalue(
        raw_lims_samples_fixture
    )

    # GIVEN a FlowCellSampleBCLConvert
    sample: IlluminaSampleIndexSettings = raw_lims_samples[0]
