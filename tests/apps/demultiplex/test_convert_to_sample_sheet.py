...
def test_convert_to_bcl2fastq_sheet(
    novaseq_bcl2fastq_sample_sheet_object: SampleSheetCreatorV1, project_dir: Path
):
...
def test_convert_to_dragen_sheet(
    novaseq_dragen_sample_sheet_object: SampleSheetCreatorV1, project_dir: Path
):
...
def test_convert_to_novaseq_x_sheet(
    novaseq_x_sample_sheet_object: SampleSheetCreatorV2, project_dir: Path
):
    """Test that a created NovaSeqX sample sheet has samples."""
    # GIVEN a sample sheet object populated with Dragen samples
    assert novaseq_x_sample_sheet_object.lims_samples

    # WHEN converting to a sample sheet
    sample_sheet: List[List[str]] = novaseq_x_sample_sheet_object.construct_sample_sheet()
