...
def test_instatiate(mutacc_config):
    ...
    assert mutacc_api.base_call == [mutacc_config['mutacc-auto']['binary_path'],
                                    '--config-file', mutacc_config['mutacc-auto']['config_path']]
...
def test_extract_reads(mutacc_auto_api, mocker):
    ...
    expected_call = mutacc_auto_api.base_call + [
        ...
        '--case', str(case),
        '--padding', str(mutacc_auto_api.mutacc_padding)
        ]
