...
def test_link_file_count(
    cg_config, link_family, simple_files_data, store: Store, tb_api: tb.TrailblazerAPI
):
...
def ensure_application_version(disk_store, application_tag="dummy_tag"):
    ...
    if not application:
        application = disk_store.add_application(
            tag=application_tag, category="wgs", percent_kth=80, description="dummy_description"
        )
