...
def flow_cell_working_directory_bcl2fastq(
    bcl2fastq_flow_cell_dir: Path, tmp_illumina_novaseq_flow_cells_directory
) -> Path:
    ...
    """
    return Path(tmp_illumina_novaseq_flow_cells_directory, bcl2fastq_flow_cell_dir.name)


@pytest.fixture(name="tmp_flow_cell_directory_bclconvert")
def flow_cell_working_directory_bclconvert(
    bcl_convert_flow_cell_dir: Path, tmp_illumina_novaseq_flow_cells_directory
) -> Path:
    """Return the path to a working directory that will be deleted after test is run.
    This is a path to a flow cell directory with the run parameters present.
    """
    return Path(tmp_illumina_novaseq_flow_cells_directory, bcl_convert_flow_cell_dir.name)
...
def tmp_flow_cells_directory_malformed_sample_sheet(
    tmp_flow_cell_name_malformed_sample_sheet: str, tmp_illumina_novaseq_flow_cells_directory
) -> Path:
    """This is a path to a flow cell directory with a sample sheet with malformed headers."""
    return Path(
        tmp_illumina_novaseq_flow_cells_directory, tmp_flow_cell_name_malformed_sample_sheet
    )


@pytest.fixture
def tmp_flow_cells_directory_ready_for_demultiplexing_bcl_convert(
    bcl_convert_flow_cell_full_name: str, tmp_illumina_novaseq_flow_cells_directory
) -> Path:
    """This is a path to a flow cell directory with the run parameters missing."""
    return Path(tmp_illumina_novaseq_flow_cells_directory, bcl_convert_flow_cell_full_name)
...
@pytest.fixture
def tmp_illumina_novaseq_demultiplexed_flow_cells_directory(
    tmp_path: Path, illumina_novaseq_demultiplexed_runs
) -> Path:
    """Return the path to a temporary demultiplex-runs directory.
    Generates a copy of the original demultiplexed-runs
    """
    original_dir = illumina_novaseq_demultiplexed_runs
...
def tmp_demultiplexed_runs_bcl2fastq_directory(
    tmp_illumina_novaseq_demultiplexed_flow_cells_directory, bcl2fastq_flow_cell_dir: Path
) -> Path:
    """Return the path to a temporary demultiplex-runs bcl2fastq flow cell directory."""
    return Path(
        tmp_illumina_novaseq_demultiplexed_flow_cells_directory, bcl2fastq_flow_cell_dir.name
    )
...
@pytest.fixture(scope="session")
def illumina_novaseq_demultiplexed_runs(demultiplex_fixtures: Path) -> Path:
    """Return the path to the demultiplexed flow cells fixture directory."""
    return Path(demultiplex_fixtures, "demultiplexed-runs")


@pytest.fixture(scope="session")
def illumina_novaseq_flow_cells_dir(demultiplex_fixtures: Path) -> Path:
...
@pytest.fixture(scope="session")
def illumina_novaseq_flow_cells_demux_all_dir(demultiplex_fixtures: Path) -> Path:
    """Return the path to the sequenced flow cells fixture directory."""
    return Path(demultiplex_fixtures, "flow_cells_demux_all")


@pytest.fixture(scope="session")
def illumina_novaseq_demux_results_not_finished_dir(demultiplex_fixtures: Path) -> Path:
...
@pytest.fixture
def novaseq_x_flow_cell_directory(tmp_illumina_novaseq_flow_cells_directory) -> Path:
    return Path(tmp_illumina_novaseq_flow_cells_directory, "20231108_LH00188_0028_B22F52TLT3")
...
def hiseq_x_single_index_flow_cell_dir(
    illumina_novaseq_flow_cells_dir, hiseq_x_single_index_flow_cell_name: str
) -> Path:
    """Return the path to a HiSeqX flow cell."""
    return Path(illumina_novaseq_flow_cells_dir, hiseq_x_single_index_flow_cell_name)


@pytest.fixture(scope="session")
def hiseq_x_dual_index_flow_cell_dir(
    illumina_novaseq_flow_cells_dir, hiseq_x_dual_index_flow_cell_name: str
) -> Path:
    """Return the path to a HiSeqX flow cell."""
    return Path(illumina_novaseq_flow_cells_dir, hiseq_x_dual_index_flow_cell_name)


@pytest.fixture(scope="session")
def hiseq_2500_dual_index_flow_cell_dir(
    illumina_novaseq_flow_cells_dir, hiseq_2500_dual_index_flow_cell_name: str
) -> Path:
    """Return the path to a HiSeq2500 flow cell."""
    return Path(illumina_novaseq_flow_cells_dir, hiseq_2500_dual_index_flow_cell_name)


@pytest.fixture(scope="session")
def hiseq_2500_custom_index_flow_cell_dir(
    illumina_novaseq_flow_cells_dir, hiseq_2500_custom_index_flow_cell_name: str
) -> Path:
    """Return the path to a HiSeq2500 flow cell."""
...
def bcl2fastq_flow_cell_dir(
    ...
    return Path(illumina_novaseq_flow_cells_dir, bcl2fastq_flow_cell_full_name)


@pytest.fixture(scope="session")
def bcl_convert_flow_cell_dir(
    illumina_novaseq_flow_cells_dir, bcl_convert_flow_cell_full_name: str
) -> Path:
...
def demultiplexed_flow_cell(
    ...
    return Path(illumina_novaseq_demultiplexed_runs, bcl2fastq_flow_cell_full_name)
...
def tmp_empty_demultiplexed_runs_directory(
    ...
    return Path(tmp_illumina_novaseq_demultiplexed_flow_cells_directory, "empty")


@pytest.fixture
def novaseqx_demultiplexed_flow_cell(
    illumina_novaseq_demultiplexed_runs, novaseq_x_flow_cell_full_name: str
):
    """Return the path to a demultiplexed NovaSeqX flow cell."""
    return Path(illumina_novaseq_demultiplexed_runs, novaseq_x_flow_cell_full_name)
