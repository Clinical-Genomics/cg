...
from cg.constants.nanopore_files import (
    NanoporeDirsAndFiles,
)
...
@pytest.fixture
def tmp_illumina_flow_cells_directory(
    tmp_path: Path,
...
def tmp_illumina_flow_cells_demux_all_directory(
    tmp_path: Path,
    illumina_demux_all_directory: Path,
) -> Path:
    """
    Return the path to a temporary flow cells directory with flow cells ready for demultiplexing.
    Generates a copy of the original flow cells directory.
...
def flow_cell_working_directory_bclconvert(
    ...
    return Path(
        tmp_illumina_flow_cells_directory,
        bcl_convert_flow_cell_dir.name,
    )
...
def tmp_flow_cell_without_run_parameters_path(
    ...
    return Path(
        tmp_broken_flow_cells_directory,
...
def tmp_flow_cells_directory_malformed_sample_sheet(
    tmp_flow_cell_name_malformed_sample_sheet: str,
    tmp_illumina_flow_cells_directory,
) -> Path:
    """This is a path to a flow cell directory with a sample sheet with malformed headers."""
    return Path(
        tmp_illumina_flow_cells_directory,
        tmp_flow_cell_name_malformed_sample_sheet,
    )
...
def tmp_flow_cells_directory_ready_for_demultiplexing_bcl_convert(
    ...
    return Path(
        tmp_illumina_flow_cells_directory,
        bcl_convert_flow_cell_full_name,
    )
...
def tmp_demultiplexed_runs_not_finished_flow_cells_directory(
    tmp_path: Path,
    illumina_novaseq_demux_results_not_finished_dir,
) -> Path:
    """
...
@pytest.fixture(scope="session")
def illumina_demultiplexed_runs_directory(
    demultiplex_fixtures: Path,
) -> Path:
...
@pytest.fixture(scope="session")
def illumina_flow_cells_directory(
    demultiplex_fixtures: Path,
) -> Path:
...
@pytest.fixture(scope="session")
def nanopore_flow_cells_dir(
    demultiplex_fixtures: Path,
) -> Path:
    """Return the path to the sequenced flow cells fixture directory."""
    return Path(
        demultiplex_fixtures,
        NanoporeDirsAndFiles.DATA_DIRECTORY,
    )


@pytest.fixture(scope="session")
def illumina_demux_all_directory(
    demultiplex_fixtures: Path,
) -> Path:
    """Return the path to the sequenced flow cells fixture directory."""
    return Path(
        demultiplex_fixtures,
        "flow_cells_demux_all",
    )


@pytest.fixture(scope="session")
def illumina_demux_results_not_finished_dir(
    demultiplex_fixtures: Path,
) -> Path:
    """Return the path to a dir with demultiplexing results where nothing has been cleaned."""
    return Path(
        demultiplex_fixtures,
        "demultiplexed-runs-unfinished",
    )
...
@pytest.fixture
def novaseq_x_flow_cell_directory(
    tmp_illumina_flow_cells_directory,
) -> Path:
    return Path(
        tmp_illumina_flow_cells_directory,
...
@pytest.fixture
def novaseq_x_correct_sample_sheet(
    novaseq_x_flow_cell_directory: Path,
) -> Path:
    return Path(
        novaseq_x_flow_cell_directory,
        CORRECT_SAMPLE_SHEET,
    )


@pytest.fixture
def novaseq_x_manifest_file(
...
def hiseq_x_single_index_bcl2fastq_sample_sheet_path(
    ...
    return Path(
        hiseq_x_single_index_flow_cell_dir,
        bcl2fastq_sample_sheet_file_name,
    )
...
@pytest.fixture
def hiseq_2500_custom_index_sample_sheet_path(
...
@pytest.fixture(scope="session")
def novaseq_bcl2fastq_sample_sheet_path(
    bcl2fastq_flow_cell_dir: Path,
) -> Path:
...
@pytest.fixture(scope="session")
def novaseq_bcl_convert_sample_sheet_path(
    bcl_convert_flow_cell_dir: Path,
) -> Path:
...
@pytest.fixture
def novaseq_6000_sample_sheet_with_reversed_cycles(
    sample_sheet_dir: Path,
) -> Path:
    """Return the path to a NovaSeq6000 sample sheet with reversed index2 cycles."""
    return Path(
        sample_sheet_dir,
        "novaseq_6000_sample_sheet_with_reversed_cycles.csv",
    )


@pytest.fixture
def novaseq_x_sample_sheet_with_forward_cycles(
    sample_sheet_dir: Path,
) -> Path:
    """Return the path to a NovaSeqX sample sheet with forward index2 cycles."""
    return Path(
        sample_sheet_dir,
        "novaseq_x_sample_sheet_with_forward_cycles.csv",
    )


@pytest.fixture(scope="session")
def run_parameters_wrong_instrument(
    run_parameters_dir: Path,
) -> Path:
    """Return a NovaSeqX run parameters file path with a wrong instrument value."""
    return Path(
        run_parameters_dir,
        "RunParameters_novaseq_X_wrong_instrument.xml",
    )
...
@pytest.fixture(scope="session")
def novaseq_6000_run_parameters_path(
    bcl2fastq_flow_cell_dir: Path,
) -> Path:
...
@pytest.fixture
def novaseq_x_run_parameters_path(
    novaseq_x_flow_cell_dir: Path,
) -> Path:
...
def run_parameters_missing_versions_path(
    ...
    return Path(
        run_parameters_dir,
        "RunParameters_novaseq_no_software_nor_reagent_version.xml",
    )


@pytest.fixture(name="lims_novaseq_samples_file")
def lims_novaseq_samples_file(
    raw_lims_sample_dir: Path,
) -> Path:
    """Return the path to a file with sample info in lims format."""
    return Path(
        raw_lims_sample_dir,
        "raw_samplesheet_novaseq.json",
    )


@pytest.fixture
def lims_novaseq_6000_samples_file(
    bcl2fastq_flow_cell_dir: Path,
) -> Path:
