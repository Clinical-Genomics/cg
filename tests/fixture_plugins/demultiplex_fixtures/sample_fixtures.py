...
def hiseq_x_single_index_bcl_convert_lims_samples(
    ...
    return [IlluminaIndexSettings.model_validate(sample) for sample in read_json(path)]
...
@pytest.fixture
def novaseq_x_lims_samples(novaseq_x_flow_cell_dir: Path) -> list[IlluminaIndexSettings]:
...
def bcl_convert_sample_sheet_creator(
    ...
    novaseq_x_lims_samples: list[IlluminaIndexSettings],
) -> SampleSheetCreator:
