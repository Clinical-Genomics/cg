...
@pytest.fixture(name="send_fastq_context")
def fixture_send_fastq_context(
    cg_context: CGConfig,
    fastq_case,
    ...
    """Returns a CGConfig where the meta_apis["analysis_api"] is a SendFastqAnalysisAPI and a store containing a
    fastq case"""
    _store = cg_context.status_db
    cg_context.trailblazer_api_ = tb_api
    send_fastq_analysis = SendFastqAnalysisAPI(config=cg_context, pipeline=Pipeline.FASTQ)
    ...
    cg_context.meta_apis["analysis_api"] = send_fastq_analysis
    return cg_context
