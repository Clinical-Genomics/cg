...
def test_dry_run(cli_runner, balsamic_context, caplog):
    ...
    Path(balsamic_context["BalsamicAnalysisAPI"].get_config_path(case_id)).touch(exist_ok=True)
    Path.mkdir(
        Path(balsamic_context["BalsamicAnalysisAPI"].get_analysis_finish_path(case_id)).parent, exist_ok=True
    )
    Path(balsamic_context["BalsamicAnalysisAPI"].get_analysis_finish_path(case_id)).touch(exist_ok=True)
    # WHEN dry running with dry specified
    result = cli_runner.invoke(report_deliver, [case_id, "--dry-run"], obj=balsamic_context)
...
def test_qc_option(cli_runner, balsamic_context, caplog):
    ...
    Path(balsamic_context["BalsamicAnalysisAPI"].get_analysis_finish_path(case_id)).touch(exist_ok=True)
    # WHEN dry running with dry specified
    result = cli_runner.invoke(report_deliver, [case_id, "--dry-run", option_key, option_value], obj=balsamic_context)
    # THEN command should execute successfully
    assert result.exit_code == EXIT_SUCCESS
