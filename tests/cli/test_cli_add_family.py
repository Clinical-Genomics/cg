...
def test_add_family_required(invoke_cli, disk_store: Store):
    ...
    result = invoke_cli(
        ['--database', db_uri, 'add', 'family', '--panel',
...
def test_add_family_bad_customer(invoke_cli, disk_store: Store):
    ...
    # THEN then it should complain in missing customer instead of adding a family
...
def test_add_family_bad_panel(invoke_cli, disk_store: Store):
    ...
    # THEN then it should complain in missing panel instead of adding a family
    assert result.exit_code == 1
    assert disk_store.Family.query.count() == 0
...
def test_add_family_priority(invoke_cli, disk_store: Store):
    ...
    result = invoke_cli(
        ['--database', db_uri, 'add', 'family', '--panel', panel_id,
         '--priority', priority, customer_id, name])
...
def add_panel(disk_store, panel_id='panel_test', customer_id='cust_test'):
    ...
    panel = disk_store.add_panel(customer=customer, name=panel_id, abbrev=panel_id,
        version=1.0,
                                 date=datetime.datetime.now(), genes=1)
...
def add_application(disk_store, application_tag='dummy_tag'):
    ...
    version = disk_store.add_version(application, 1, valid_from=datetime.datetime.now(),
                                     prices=prices)
