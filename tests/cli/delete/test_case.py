...
def test_delete_case_without_options(
    ...
    assert store.Family.query.count() == 1
...
def test_delete_case_without_links(
    ...
    # GIVEN a database with a case
    store: Store = base_context.status_db
    case_obj = helpers.add_case(store)
    ...
    assert result.exit_code == SUCCESS
    assert store.Family.query.count() == 0
...
def test_delete_case_with_analysis(
    ...
    store: Store = base_context.status_db
    analysis_obj = helpers.add_analysis(store)
...
def test_delete_case_with_dry_run(
    ...
    helpers.add_relationship(store=store, case=case_obj, sample=sample)
    assert store.Family.query.count() == 1
    assert store.FamilySample.query.count() == 1
    assert store.Sample.query.count() == 1
...
def test_delete_case_with_links(cli_runner: CliRunner, base_context: CGConfig, helpers):
    ...
    helpers.add_relationship(store=store, case=case_obj, sample=sample)
    assert store.Family.query.count() > 0
    assert store.FamilySample.query.count() > 0
    assert store.Sample.query.count() > 0
...
def test_delete_case_with_links_to_other_case(
    ...
    # GIVEN a database with a case
    store: Store = base_context.status_db
    case_obj = helpers.add_case(store, "first_case_linked_to_sample")
    case_id = case_obj.internal_id
    sample = helpers.add_sample(store)
    helpers.add_relationship(store=store, case=case_obj, sample=sample)
    case_obj2 = helpers.add_case(store, "second_case_linked_to_sample")
    helpers.add_relationship(store=store, case=case_obj2, sample=sample)

    assert store.Family.query.count() == 2
    assert store.FamilySample.query.count() == 2
    assert store.Sample.query.count() == 1
...
def test_delete_case_with_father_links(
    ...
    case_obj = helpers.add_case(store, "first_case_linked_to_sample")
    case_id = case_obj.internal_id
    sample_father = helpers.add_sample(store, "father")
    sample_child = helpers.add_sample(store, "child")
    helpers.add_relationship(store=store, case=case_obj, sample=sample_father)
    case_obj2 = helpers.add_case(store, "second_case_linked_to_sample")
    helpers.add_relationship(store=store, case=case_obj2, sample=sample_child, father=sample_father)

    assert store.Family.query.count() == 2
    assert store.FamilySample.query.count() == 2
    assert store.Sample.query.count() == 2
...
def test_delete_mother_case(cli_runner: CliRunner, base_context: CGConfig, helpers: StoreHelpers):
    ...
    store: Store = base_context.status_db
    case_mother = helpers.add_case(store, "case_mother")
    case_mother_id = case_mother.internal_id
    sample_mother = helpers.add_sample(store, "mother")
    sample_child = helpers.add_sample(store, "child")
    helpers.add_relationship(store=store, case=case_mother, sample=sample_mother)
    case_child = helpers.add_case(store, "case_child")
    helpers.add_relationship(
        store=store, case=case_child, sample=sample_child, mother=sample_mother
    )
...
def test_delete_trio_case(cli_runner: CliRunner, base_context: CGConfig, helpers):
    ...
    sample_father = helpers.add_sample(store, "father")
    sample_child = helpers.add_sample(store, "child")
    helpers.add_relationship(store=store, case=case_obj, sample=sample_mother)
    helpers.add_relationship(store=store, case=case_obj, sample=sample_father)
    helpers.add_relationship(
        store=store,
        case=case_obj,
    ...
    assert store.Family.query.count() == 1
    assert store.FamilySample.query.count() == 3
    assert store.Sample.query.count() == 3
