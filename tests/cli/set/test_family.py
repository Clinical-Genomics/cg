...
def test_set_family_without_options(
    ...
    case_id = helpers.add_case(store).internal_id
...
def test_set_family_bad_panel(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    ...
    assert result.exit_code != SUCCESS
    assert panel_id not in store.Family.query.first().panels


def test_set_family_panel(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    """Test to set a case using an existing panel"""
    # GIVEN a database with a case and a panel not yet added to the case
    panel_id = helpers.ensure_panel(store, "a_panel").name
    case_id = helpers.add_case(store).internal_id
    assert panel_id not in store.Family.query.first().panels
    ...
    assert result.exit_code == SUCCESS
    assert panel_id in store.Family.query.first().panels


def test_set_family_priority(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    """Test that the added case gets the priority we send in"""
    # GIVEN a database with a case
    case_id = helpers.add_case(store).internal_id
    priority = "priority"
    assert store.Family.query.first().priority_human != priority
...
def test_set_family_customer(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    """Test to set a case using an existing customer"""
    # GIVEN a database with a case and a customer not yet on the case
    customer_id = helpers.ensure_customer(store, customer_id="a_customer").internal_id
...
def test_set_family_bad_data_analysis(
    ...
    assert result.exit_code != SUCCESS
    assert str(data_analysis) != store.Family.query.first().data_analysis
...
def test_set_family_bad_data_delivery(
    ...
    assert result.exit_code != SUCCESS
    assert str(data_delivery) != store.Family.query.first().data_delivery
