...
def test_skip_lims(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample

    sample_obj = helpers.add_sample(store, gender=Gender.FEMALE)
    key = "name"
    new_value = "new_value"
...
def test_set_sample(cli_runner: CliRunner, base_context: CGConfig, store: Store, key, helpers):
    # GIVEN a database with a sample
...
def test_set_boolean_sample(
    cli_runner: CliRunner, base_context: CGConfig, store: Store, new_value, helpers
):
    # GIVEN a database with a sample
...
def test_sex(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
...
def test_priority_text(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_obj = helpers.add_sample(store, gender=Gender.FEMALE)
    key = "priority"
...
def test_priority_number(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_obj = helpers.add_sample(store, gender=Gender.FEMALE)
    key = "priority"
...
def test_sample_comment(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
...
def test_sample_comment_append(
    cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers
):
    # GIVEN a database with a sample with a comment
    old_value = "test comment"
    sample_obj = helpers.add_sample(
        store, gender=Gender.FEMALE, comment=f"2022-06-13 10:16-test.user: {old_value}"
    )
    key = "comment"
...
def test_invalid_customer(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_id = helpers.add_sample(store).internal_id
    customer_id = "dummy_customer_id"
    assert store.Sample.query.first().customer.internal_id != customer_id
...
def test_customer(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample and two customers
    sample_id = helpers.add_sample(store).internal_id
    customer_id = helpers.ensure_customer(store, "another_customer").internal_id
    assert store.Sample.query.first().customer.internal_id != customer_id
...
def test_downsampled_to(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_id = helpers.add_sample(store).internal_id
    downsampled_to = 111111
    assert store.Sample.query.first().downsampled_to != downsampled_to
...
def test_reset_downsampled_to(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_id = helpers.add_sample(store).internal_id
    downsampled_to = 0
    assert store.Sample.query.first().downsampled_to != downsampled_to
    ...
    # THEN the value should have been set on the sample
    assert result.exit_code == EXIT_SUCCESS
    assert not store.Sample.query.first().downsampled_to


def test_invalid_application(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
    # GIVEN a database with a sample
    sample_id = helpers.add_sample(store).internal_id
    application_tag = "dummy_application"
    assert store.Sample.query.first().application_version.application.tag != application_tag
...
def test_application(cli_runner: CliRunner, base_context: CGConfig, store: Store, helpers):
