...
def test_create_manifest_files_true(
    ...
    second_flowcell_directory = Path(
        shutil.copytree(
            ...
            Path(
                tmp_flow_cells_directory_ready_for_demultiplexing_bcl_convert.parent,
                "flowcell_2",
            ),
        )
    )
    ...
    cli_runner.invoke(
        ...
        args=[
            "--source-directory",
            first_flowcell_directory.parent.as_posix(),
        ],
    )

    for flow_cell in [
        first_flowcell_directory,
        second_flowcell_directory,
    ]:
        manifest_file = Path(
            flow_cell,
        ...
        assert_manifest_file_contains_all_files(
            ...
            directory_of_interest=flow_cell,
        )
...
def test_create_manifest_files_true_nanopore_data(
    ...
    tmp_nanopore_directory = Path(
        shutil.copytree(
            nanopore_flow_cells_dir,
            Path(
                tmp_path,
                nanopore_flow_cells_dir.name,
            ).as_posix(),
        )
    )
    ...
    for sample_directory in sample_directories:
        ...
        assert_manifest_file_contains_all_files(
            manifest_file=manifest_file,
            directory_of_interest=sample_directory,
        )


def assert_manifest_file_contains_all_files(
    manifest_file: Path,
    directory_of_interest: Path,
) -> None:
