...
def test_get_report_case(click_context, case_id):
    ...
    case_obj = get_report_case(click_context, case_id)

    # THEN a valid case object should be returned
    assert case_obj.internal_id == case_id


def test_get_report_case(click_context, caplog):
    ...
    with pytest.raises(click.exceptions.Abort):
        get_report_case(click_context, "not a case ID")

    assert "Invalid case ID" in caplog.text


def test_get_report_api(click_context, cg_context, case_id):
    ...
    report_api = get_report_api(click_context, case_obj)
...
def test_get_report_api_pipeline(click_context):
    ...
    report_api = get_report_api_pipeline(click_context, pipeline)
...
def test_get_report_analysis_started(click_context, case_id):
