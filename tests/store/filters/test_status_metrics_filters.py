...
from cg.store.filters.status_metrics_filters import (
    get_above_q30_threshold,
    get_by_flow_cell_name,
    get_by_flow_cell_sample_internal_id_and_lane,
    get_by_sample_internal_id,
    get_total_read_count_for_sample,
)
...
def test_get_metrics_for_flow_cell_sample_internal_id_and_lane(
    ...
    metrics_query: Query = get_by_flow_cell_sample_internal_id_and_lane(
        metrics=metrics,
...
def test_get_metrics_by_flow_cell_name(store_with_sequencing_metrics: Store, flow_cell_name: str):
    # GIVEN a Store with sequencing metrics
    metrics: Query = store_with_sequencing_metrics._get_query(table=SampleLaneSequencingMetrics)

    # WHEN getting metrics for a flow cell name
    metrics_query: Query = get_by_flow_cell_name(metrics=metrics, flow_cell_name=flow_cell_name)
...
def test_get_metrics_by_sample_internal_id(store_with_sequencing_metrics: Store, sample_id: str):
    # GIVEN a Store with sequencing metrics
    metrics: Query = store_with_sequencing_metrics._get_query(table=SampleLaneSequencingMetrics)

    # WHEN getting metrics for a sample internal id
    metrics_query: Query = get_by_sample_internal_id(sample_internal_id=sample_id, metrics=metrics)
...
def test_get_above_q30_threshold(store_with_sequencing_metrics: Store):
    # GIVEN a Store with sequencing metrics
    metrics: Query = store_with_sequencing_metrics._get_query(table=SampleLaneSequencingMetrics)
    ...
    filtered_metrics: Query = get_above_q30_threshold(
        metrics=metrics,
...
def test_get_metrics_by_flow_cell_name(store_with_sequencing_metrics: Store, flow_cell_name: str):
    ...
    filtered_metrics: Query = get_by_flow_cell_name(metrics=metrics, flow_cell_name=flow_cell_name)
