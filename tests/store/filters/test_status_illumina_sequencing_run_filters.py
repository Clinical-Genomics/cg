...
from cg.store.filters.status_illumina_sequencing_run_filters import (
    ...
    filter_by_device_internal_id_search,
    filter_by_entry_id,
)
...
def test_filter_by_run_internal_id_search(
    store_with_illumina_sequencing_data: Store,
    novaseq_x_flow_cell_id: str,
    seven_canonical_flow_cells: list[IlluminaRunDirectoryData],
):
    """Test to filter sequencing runs by run internal id search."""
    # GIVEN a store with Illumina Sequencing Runs for canonical flow cells
    sequencing_runs: Query = store_with_illumina_sequencing_data._get_query(
        table=IlluminaSequencingRun
    )
    ...
    # WHEN filtering sequencing runs by searching a part of the device internal id
    filtered_runs: Query = filter_by_device_internal_id_search(
        runs=sequencing_runs, id_search=novaseq_x_flow_cell_id[:4]
    )
