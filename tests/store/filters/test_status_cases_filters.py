...
def test_get_cases_has_sequence(store: Store, helpers: StoreHelpers, timestamp_now: datetime):
    ...
    # GIVEN a case
    test_case = helpers.add_case(store)
    ...
    # GIVEN a cases Query
    cases: Query = store._get_outer_join_cases_with_analyses_query()
...
def test_get_cases_has_sequence_when_external(store: Store, helpers: StoreHelpers):
...
def test_get_cases_has_sequence_when_not_sequenced(store: Store, helpers: StoreHelpers):
    """Test that no case is returned when there is a cases with sample that has not been sequenced."""

    # GIVEN a sequenced sample
    test_sample: Sample = helpers.add_sample(store, sequenced_at=None)
...
def test_get_cases_has_sequence_when_not_external_nor_sequenced(
    ...
    # GIVEN a sequenced sample
    test_sample: Sample = helpers.add_sample(store, sequenced_at=None, is_external=False)
...
def test_get_cases_with_loqusdb_supported_pipeline(
    ...
    test_mip_case: Family = helpers.add_case(store, data_analysis=Pipeline.MIP_DNA)
    test_mip_case.customer.loqus_upload = True
    test_fluffy_case: Family = helpers.add_case(store, name="test", data_analysis=Pipeline.FLUFFY)
    test_fluffy_case.customer.loqus_upload = True

    # GIVEN a database with a case with one sequenced samples for specified analysis
    store.relate_sample(test_mip_case, test_sample, PhenotypeStatus.UNKNOWN)
    store.relate_sample(test_fluffy_case, test_sample, PhenotypeStatus.UNKNOWN)
...
def test_get_cases_with_loqusdb_supported_sequencing_method(
    ...
    test_sample_wes: Sample = helpers.add_sample(
        store, sequenced_at=timestamp_now, application_type=SequencingMethod.WES
    )

    # GIVEN a MIP-DNA associated test case
    test_case_wes: Family = helpers.add_case(store, data_analysis=Pipeline.MIP_DNA)
    store.relate_sample(test_case_wes, test_sample_wes, PhenotypeStatus.UNKNOWN)
...
def test_get_cases_with_loqusdb_supported_sequencing_method_empty(
    ...
    test_sample_wts: Sample = helpers.add_sample(
        store, name="sample_wts", sequenced_at=timestamp_now, is_rna=True
    )

    # GIVEN a MIP-DNA associated test case
    test_case_wts: Family = helpers.add_case(store, data_analysis=Pipeline.MIP_DNA)
    store.relate_sample(test_case_wts, test_sample_wts, PhenotypeStatus.UNKNOWN)
...
def test_get_cases_for_analysis(store: Store, helpers: StoreHelpers, timestamp_now: datetime):
...
def test_get_cases_for_analysis_when_sequenced_sample_and_no_analysis(
    ...
    # GIVEN a sequenced sample
    test_sample: Sample = helpers.add_sample(store, sequenced_at=timestamp_now, is_external=False)
...
def test_get_cases_for_analysis_when_cases_with_no_action_and_new_sequence_data(
    ...
    # GIVEN a completed analysis
    test_analysis: Analysis = helpers.add_analysis(store, pipeline=Pipeline.MIP_DNA)
...
def test_get_cases_with_scout_data_delivery(
    ...
    # GIVEN a sequenced sample
    test_sample: Sample = helpers.add_sample(store)

    # GIVEN a case with Scout as data delivery
    test_case = helpers.add_case(store, data_delivery=DataDelivery.FASTQ_ANALYSIS_SCOUT)
...
def test_get_report_supported_data_delivery_cases(
    ...
    test_case = helpers.add_case(store, data_delivery=DataDelivery.FASTQ_ANALYSIS_SCOUT)
    test_invalid_case = helpers.add_case(store, name="test", data_delivery=DataDelivery.FASTQ)

    # GIVEN a database with the test cases
    store.relate_sample(test_case, test_sample, PhenotypeStatus.UNKNOWN)
    store.relate_sample(test_invalid_case, test_sample, PhenotypeStatus.UNKNOWN)
...
def test_get_inactive_analysis_cases(store: Store, helpers: StoreHelpers):
    ...
    # GIVEN a cases Query
    cases: Query = store._get_query(table=Family)
...
def test_get_inactive_analysis_cases_when_on_hold(store: Store, helpers: StoreHelpers):
    """Test that an inactivated case is returned when there is case which has action set to hold."""

    # GIVEN a case
    test_case = helpers.add_case(store, action=CaseActions.HOLD)
...
def test_get_inactive_analysis_cases_when_not_completed(store: Store, helpers: StoreHelpers):
    """Test that no case is returned when there is case which action set to running."""

    # GIVEN a case
    helpers.add_case(store, action=CaseActions.RUNNING)
...
def test_get_new_cases(store: Store, helpers: StoreHelpers, timestamp_in_2_weeks: datetime):
