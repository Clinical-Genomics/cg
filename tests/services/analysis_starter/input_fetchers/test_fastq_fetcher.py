...
def test_ensure_files_are_ready_missing_raw_data_files():
    # GIVEN a sample
    sample: Sample = create_autospec(Sample)
    # GIVEN a case with a workflow that does not do partial analyses
...
def test_ensure_files_are_ready_missing_raw_data_files_allowed_for_partial_analysis():
    # GIVEN a sample with no reads
    sample: Sample = create_autospec(Sample, has_reads=False)
    # GIVEN a case with a workflow that does do partial analyses
    case: Case = create_autospec(Case, data_analysis=Workflow.MICROSALT, samples=[sample])
    status_db: Store = create_autospec(Store)
    status_db.get_case_by_internal_id = Mock(return_value=case)
