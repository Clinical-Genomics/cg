...
def create_fastqs_directory(
    number_forward_reads: int, number_reverse_reads: int, tmp_path: Path, sample_id: str
):
...
def fastqs_dir(tmp_path: Path, sample_id: str) -> Path:
    return create_fastqs_directory(
        number_forward_reads=3, number_reverse_reads=3, tmp_path=tmp_path, sample_id=sample_id
    )
...
def fastqs_forward(tmp_path: Path, sample_id: str) -> Path:
    ...
    return create_fastqs_directory(
        number_forward_reads=3, number_reverse_reads=0, tmp_path=tmp_path, sample_id=sample_id
    )
