...
def test_demux_results_instance(demultiplexed_flowcell: Path, flowcell_object: Flowcell):
    ...
    demux_results = DemuxResults(demux_dir=demultiplexed_flowcell, flowcell=flowcell_object)
...
def test_parse_log(demux_results: DemuxResults):
    # GIVEN som demux results

    # WHEN parsed log file parameters
    log_file_parameters: LogfileParameters = demux_results.get_logfile_parameters()
