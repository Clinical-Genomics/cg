...
class FetchFastqDeliveryFileTagsService(FetchDeliveryFileTagsService):

    def fetch_tags(self, workflow: str) -> DeliveryFileTags:
        """Get the fastq tags for the files that need to be delivered for a workflow."""
        return DeliveryFileTags(sample_tags=PIPELINE_ANALYSIS_TAG_MAP[workflow]["sample_tags"])
