...
def build_submitter(lims: FlaskLims, status_db: Store, order_type: OrderType) -> OrderSubmitter:
    ...
    return submitter_class(
        order_validation_service=validation_service(),
        order_store_service=store_service(status_db),
        order_lims_service=lims_service(lims),
    )


def setup_order_submitter_registry(lims: FlaskLims, status_db: Store) -> OrderSubmitterRegistry:
    ...
    registry.register(
        ...
        order_submitter=build_submitter(
            lims=lims, status_db=status_db, order_type=OrderType.BALSAMIC
        ),
    )
    return registry
