...
def is_normal_only_wgs(case: BalsamicCase | BalsamicUmiCase, store: Store) -> bool:
    if len(case.samples) == 1:
        return _is_sample_wgs_normal(sample = case.samples[0], store = store)
    else:
        return False

def _is_sample_wgs_normal(sample: BalsamicSample | BalsamicUmiSample | ExistingSample, store: Store) -> bool:
    if sample.is_new:
        application: Application = store.get_application_by_tag(sample.application)
        return application.prep_category == SeqLibraryPrepCategory.WHOLE_GENOME_SEQUENCING and not sample.tumour
    else:
        db_sample: Sample = store.get_sample_by_internal_id(sample.internal_id)
        return db_sample.prep_category == SeqLibraryPrepCategory.WHOLE_GENOME_SEQUENCING and not db_sample.is_tumour
