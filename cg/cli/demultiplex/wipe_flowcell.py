...
class WipeDemuxAPI:
    ...
    def _wipe_fastq_tags_housekeeper(
        self, housekeeper_api: HousekeeperAPI, status_db: Store, samples: List[str]
    ) -> None:
        """Wipe the presence of a flowcell, based on samples related to flowcell and the fastq-tags, in Housekeeper."""
        samples_to_delete = []

        for sample in samples:
            if housekeeper_api.check_for_files(tags=["fastq"]):
        ...
        if samples_to_delete and self.exit_code == EXIT_SUCCESS:
            for sample in samples_to_delete:
                housekeeper_api.delete_files(dry_run=self.dry_run, tags=["fastq"], bundle=sample)
            return
    ...
    def _active_samples_on_flowcell(self) -> bool:
