...
@click.option(
    "-c",
    "--case-id",
    help="internal case id, leave empty to process all",
)
@click.option(
    "-d",
    "--days-ago",
...
@click.option(
    ...
    type=str,
    multiple=True,
    help="Filter on customers",
)
@click.option(
    "--dry-run",
    is_flag=True,
    help="only print cases to be processed",
)
@click.pass_obj
def process_solved(
    context: CGConfig,
    ...
    mutacc_upload_api = UploadToMutaccAPI(
        scout_api=scout_api,
