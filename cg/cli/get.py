...
def sample(context: click.Context, families: bool, flowcells: bool, sample_ids: List[str]):
    ...
    for sample_id in sample_ids:
        ...
        sample_obj = context.obj["clinical_db"].sample(sample_id)
...
def family(
    ...
    if name:
        customer_obj = context.obj["clinical_db"].customer(customer)
        if customer_obj is None:
            LOG.error(f"{customer}: customer not found")
            context.abort()
        families = context.obj["clinical_db"].families(
            customer=customer_obj, enquiry=family_ids[-1]
        )
    else:
        families = []
...
def flowcell(context: click.Context, samples: bool, flowcell_id: str):
    ...
    flowcell_obj = context.obj["clinical_db"].flowcell(flowcell_id)
