...
@click.option(
    "-i",
    "--identifier",
    help="made a guess what type you are looking for",
)
@click.pass_context
def get(context: click.Context, identifier: str | None):
    ...
    if identifier:
        if re.match(
            r"^[A-Z]{3}[0-9]{4,5}[A-Z]{1}[1-9]{1,3}$",
            identifier,
        ):
            context.invoke(
                get_sample,
                sample_ids=[identifier],
            )
...
@click.option(
    "--cases/--no-cases",
    default=True,
    help="Display related cases",
)
@click.option(
    "-hf",
    "--hide-flow-cell",
    is_flag=True,
    help="Hide related flow cells",
)
    ...
    for sample_id in sample_ids:
        ...
        if not hide_flow_cell:
            for sample_flow_cell in existing_sample.flowcells:
                ...
                context.invoke(
                    get_flow_cell,
                    flow_cell_id=sample_flow_cell.name,
                    samples=False,
                )
...
@click.option(
    "-c",
    "--customer-id",
    help="internal id for customer to filter by",
)
@click.option(
    "-n",
    "--name",
    is_flag=True,
    help="search family by name",
)
@click.option(
    "--samples/--no-samples",
    default=True,
    help="display related samples",
)
@click.option(
    "--relate/--no-relate",
    default=True,
    help="display relations to samples",
)
@click.option(
    "--analyses/--no-analyses",
    default=True,
    help="display related analyses",
)
...
def get_case(
    ...
    for status_db_case in status_db_cases:
        ...
        if relate:
            context.invoke(
                get_case_relations,
                case_id=status_db_case.internal_id,
            )
        if samples:
            sample_ids: list[str] = [
