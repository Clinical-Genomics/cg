...
def customer(
    ...
    existing = context.obj["clinical_db"].customer(internal_id)
    if existing:
        LOG.error(f"{existing.name}: customer already added")
        context.abort()

    customer_group = context.obj["clinical_db"].customer_group(customer_group_id)
    if not customer_group:
        customer_group = context.obj["clinical_db"].add_customer_group(
            internal_id=internal_id, name=name
        )

    new_customer = context.obj["clinical_db"].add_customer(
    ...
    context.obj["clinical_db"].add_commit(new_customer)
...
def user(context, admin, customer_id, email, name):
    ...
    customer_obj = context.obj["clinical_db"].customer(customer_id)
    existing = context.obj["clinical_db"].user(email)
    if existing:
        LOG.error(f"{existing.name}: user already added")
        context.abort()
    new_user = context.obj["clinical_db"].add_user(customer_obj, email, name, is_admin=admin)
    context.obj["clinical_db"].add_commit(new_user)
...
def relationship(context, mother, father, status, family_id, sample_id):
    ...
    status_db = context.obj["clinical_db"]
    mother_obj = None
    father_obj = None
