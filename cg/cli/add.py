...
def customer(
    ...
    if not customer_group:
        customer_group = context.obj["db"].add_customer_group(
...
@click.option(
    "-d", "--downsampled", type=int, help="how many reads is the sample downsampled to?"
)
...
@click.pass_context
def sample(
    context, lims_id, downsampled, sex, order, application, priority, customer_id, name
):
...
@click.option(
    "-p", "--panel", "panels", multiple=True, required=True, help="default gene panels"
)
@click.argument("customer_id")
@click.argument("name")
@click.pass_context
