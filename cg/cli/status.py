...
@click.option(
    "--verbose", is_flag=True, help="show status information otherwise left out"
)
...
@click.option(
    "--case-action", type=click.Choice(FAMILY_ACTIONS), help="filter by case action"
)
@click.option(
    "--progress-status",
...
@click.option(
    "--priority", type=click.Choice(PRIORITY_OPTIONS), help="filter by priority"
)
...
@click.option(
    "-r", "--only-received", is_flag=True, help="only completely received cases"
)
@click.option(
    "-R", "--exclude-received", is_flag=True, help="exclude completely received cases"
)
@click.option(
    "-p", "--only-prepared", is_flag=True, help="only completely prepared cases"
)
@click.option(
    "-P", "--exclude-prepared", is_flag=True, help="exclude completely prepared cases"
)
@click.option(
    "-s", "--only-sequenced", is_flag=True, help="only completely sequenced cases"
)
@click.option(
    "-S", "--exclude-sequenced", is_flag=True, help="exclude completely sequenced cases"
)
...
@click.option(
    "-D", "--exclude-delivered", is_flag=True, help="exclude LIMS delivered cases"
)
@click.option(
    "--dr",
    "--only-delivery-reported",
    is_flag=True,
    help="only delivery reported cases",
)
@click.option(
    "--DR",
    "--exclude-delivery-reported",
    is_flag=True,
    help="exclude delivery " "reported cases",
)
@click.option(
    "-i", "--only-invoiced", is_flag=True, help="only completely invoiced cases"
)
@click.option(
    "-I", "--exclude-invoiced", is_flag=True, help="exclude completely invoiced cases"
)
def cases(
    context,
    output_type,
    ...
    for case in records:
        ...
        if output_type == "bool":
            ...
            if case.get("analysis_completed_at"):
                ...
                analysed = (
                    f"{analysed_bool}{present_string(case, 'case_action', verbose)}"
                )

            uploaded = present_bool(case, "analysis_uploaded_bool", verbose)
            delivered = present_bool(case, "samples_delivered_bool", verbose)
            delivery_reported = present_bool(
                case, "analysis_delivery_reported_bool", verbose
            )
            invoiced = present_bool(case, "samples_invoiced_bool", verbose)

        elif output_type == "count":
            received = (
                f"{case.get('samples_received')}/{case.get('samples_to_receive')}"
            )
            prepared = (
                f"{case.get('samples_prepared')}/{case.get('samples_to_prepare')}"
            )
            sequenced = (
                f"{case.get('samples_sequenced')}/{case.get('samples_to_sequence')}"
            )
            flowcell = f"{case.get('flowcells_on_disk')}/{case.get('total_samples')}"

            if case.get("analysis_completed_at"):
                analysed = (
                    f"{present_date(case, 'analysis_completed_at', verbose, show_time)}"
                )
            elif case.get("analysis_status"):
                analysed = (
            ...
            delivered = (
                f"{case.get('samples_delivered')}/{case.get('samples_to_deliver')}"
            )
            delivery_reported = present_date(
                case, "analysis_delivery_reported_at", verbose, show_time
            )
            invoiced = (
                f"{case.get('samples_invoiced')}/{case.get('samples_to_invoice')}"
            )
    ...
    for i, _ in enumerate(case_header):
        if case_header[i] != CASE_HEADERS_LONG[i]:
            header_description = (
                f"{header_description} {case_header[i]}={CASE_HEADERS_LONG[i]}"
            )
