...
@click.option(
    ...
    "dry_run",
    is_flag=True,
    help="Test run, no changes to the " "database",
)
...
def application(context, excel_path, signature, sheet_name, dry_run):
    ...
    import_applications(
        context.obj["status"], excel_path, signature, dry_run, sheet_name
    )
...
@click.option(
    ...
    "skip_missing",
    is_flag=True,
    help="continue despite missing " "applications",
)
@click.pass_context
def application_version(context, excel_path, signature, dry_run, skip_missing):
    ...
    import_application_versions(
        context.obj["status"], excel_path, signature, dry_run, skip_missing
    )
...
@click.option(
    ...
    "--activate",
    is_flag=True,
    help="Activate archived tags found in the " "orderform",
)
@click.option(
    "-i",
    "--inactivate",
    is_flag=True,
    help="Inactivate tags not found in the orderform",
)
@click.pass_context
def apptag(
    context,
