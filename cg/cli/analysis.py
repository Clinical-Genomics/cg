...
@START_WITH_PROGRAM
@click.option(
    "-f", "--family", "family_id", help="family to prepare and start an analysis for"
)
@click.pass_context
    ...
    for link_obj in link_objs:
        ...
        if (
            ...
            context.obj["api"].link_sample(
                BalsamicFastqHandler(context.obj),
            ...
            or "mip-rna" in link_obj.sample.data_analysis.lower()
        ):
            mip_fastq_handler = MipFastqHandler(
...
def start(
    ...
    if context.obj["tb"].analyses(family=family_obj.internal_id, temp=True).first():
        ...
        context.obj["api"].start(
            family_obj, priority=priority, email=email, start_with=start_with
        )
