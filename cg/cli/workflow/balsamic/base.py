...
def config_case(context, dry, target_bed, umi_trim_length, quality_trim,
    ...
    for link_obj in link_objs:
        ...
        for file_obj in file_objs:
            ...
            with context.obj["gzipper"].open(file_obj.full_path) as handle:
                ...
                header_info = context.obj["fastq_api"].parse_header(
                    header_line)
            data = {
        ...
        if not target_bed:
            target_bed_filename = get_target_bed_from_lims(
                context.obj["lims_api"], context.obj["store_api"],
                link_obj.sample.internal_id)
    ...
    process = context.obj["balsamic_api"].config_case(arguments=arguments,
                                                      dry=dry)
    return process
...
@click.argument("case_id")
@click.pass_context
def run(context, dry, run_analysis, case_id, analysis_type, config_path, email,
        priority):
    """Generate a config for the case_id."""
