...
def get_document_title(
    workbook: xlrd.book.Book, orderform_sheet: xlrd.sheet.Sheet
) -> str:
...
def parse_sample(raw_sample):
    ...
    sample = {
        ...
        "panels": (
            raw_sample["UDF/Gene List"].split(";")
            if raw_sample.get("UDF/Gene List")
            else None
        ),
        "pool": raw_sample.get("UDF/pool name"),
        "post_formalin_fixation_time": raw_sample.get(
