...
class OrderHandler:
    ...
    def submit_project(
        self, project_name: str, samples: List[dict], researcher_id: str = "3"
    ):
        """Parse Scout project."""
        ...
        for container in containers:
            for sample in container["samples"]:
                LOG.debug(
                    "%s: adding sample to container: %s",
                    sample["name"],
                    container["name"],
                )
        ...
        sample_map = self.save_samples(
            sample_details, map_samples=process_reagentlabels
        )
            ...
            for container_name, samples in containers.items():
                ...
                for sample_data in samples:
                    ...
                    if sample_data["container"] == "96 well plate" and location is None:
                        message = (
                            f"missing 'well_position' for sample: {sample_data['name']}"
                        )
    ...
    def group_containers(samples):
        ...
        for sample_data in samples:
            if sample_data["container"] == "Tube":
                ...
                container_name = (
                    sample_data.get("container_name") or sample_data["name"]
                )
