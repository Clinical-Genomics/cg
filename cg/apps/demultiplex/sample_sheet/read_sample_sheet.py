...
def validate_samples_are_unique(samples: list[IlluminaSampleIndexSettings]) -> None:
...
def validate_samples_unique_per_lane(samples: list[IlluminaSampleIndexSettings]) -> None:
    """Validate that each sample only exists once per lane in a sample sheet."""
    sample_by_lane: dict[int, list[IlluminaSampleIndexSettings]] = get_samples_by_lane(samples)
...
) -> dict[int, list[IlluminaSampleIndexSettings]]:
    """Group and return samples by lane."""
    LOG.debug("Order samples by lane")
    sample_by_lane: dict[int, list[IlluminaSampleIndexSettings]] = {}
    for sample in samples:
...
def get_samples_from_content(
...
) -> list[IlluminaSampleIndexSettings]:
    """
    ...
    adapter = TypeAdapter(list[IlluminaSampleIndexSettings])
