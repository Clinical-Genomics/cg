...
class FlowCellSampleBcl2Fastq(FlowCellSample):
    ...
    def _pad_indexes_if_necessary(
        self, index_cycles: int, perform_reverse_complement: bool
    ) -> None:
        ...
        if is_padding_needed(index_cycles=index_cycles, sample_index_length=index_length):
            ...
            self.index2 = pad_index_two(
                index_string=self.index2, reverse_complement=perform_reverse_complement
            )
            return
    ...
    def process_sample_for_sample_sheet(self, run_parameters: RunParameters, **kwargs):
        ...
        self._pad_indexes_if_necessary(
            index_cycles=run_parameters.index_length,
            perform_reverse_complement=reverse_index2,
        )
        if reverse_index2:
