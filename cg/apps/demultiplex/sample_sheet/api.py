...
class SampleSheetAPI:
    ...
    def create_all_sample_sheets(self):
        """Create sample sheets for all flow cells."""
        for flow_cell_dir in self.flow_cell_runs_dir.iterdir():
            try:
                self.get_or_create_sample_sheet(flow_cell_dir.name, bcl_converter=BclConverter.BCL2FASTQ)
