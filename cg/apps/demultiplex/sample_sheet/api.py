...
class SampleSheetAPI:
    ...
    def _get_flow_cell(
        ...
        """
        flow_cell_path: Path = Path(
            self.flow_cell_runs_dir,
            flow_cell_name,
        )
    ...
    def _use_sample_sheet_from_housekeeper(self, flow_cell: FlowCellDirectoryData) -> None:
        ...
        try:
            ...
            if not self.dry_run:
                link_or_overwrite_file(
                    src=sample_sheet_path,
                    dst=flow_cell.sample_sheet_path,
                )
    ...
    def _get_sample_sheet_content(self, flow_cell: FlowCellDirectoryData) -> list[list[str]]:
        ...
        if not lims_samples:
            ...
            LOG.warning(message)
            raise SampleSheetError(message)
        return create_sample_sheet_content(
            flow_cell=flow_cell,
    ...
    def _create_sample_sheet_file(self, flow_cell: FlowCellDirectoryData) -> None:
        ...
        if not self.force:
            self.validator.validate_sample_sheet_from_content(
                ...
                bcl_convert=flow_cell.bcl_converter,
            )
