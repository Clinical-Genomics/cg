...
class BclConvertMetricsParser:
    def __init__(
        ...
        self.quality_metrics_path: Path = get_file_in_directory(
            directory=self.bcl_convert_demultiplex_dir,
            file_name=QUALITY_METRICS_FILE_NAME,
        )
        self.demux_metrics_path: Path = get_file_in_directory(
            directory=self.bcl_convert_demultiplex_dir,
            file_name=DEMUX_METRICS_FILE_NAME,
        )
        self.adapter_metrics_path: Path = get_file_in_directory(
            directory=self.bcl_convert_demultiplex_dir,
            file_name=ADAPTER_METRICS_FILE_NAME,
        )
    ...
    def calculate_mean_read_pair_q30_bases_percent(
        cls,
        metrics: list[BclConvertQualityMetrics],
    ) -> float:
        ...
        return (
            round(
                mean_read_pair_q30_bases_percent / SCALE_TO_READ_PAIRS,
                2,
            )
            * 100
        )
    ...
    @classmethod
    def calculate_mean_quality_score(
        cls,
        metrics: list[BclConvertQualityMetrics],
    ) -> float:
