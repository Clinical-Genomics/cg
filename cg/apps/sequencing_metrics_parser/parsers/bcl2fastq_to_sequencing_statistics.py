...
def get_sequencing_metrics_from_bcl2fastq(stats_json_path: str):
    ...
    for conversion_result in raw_sequencing_metrics.conversion_results:
        ...
        for demux_result in conversion_result.demux_results:
            lanes_mean_quality_score = calculate_lane_mean_quality_score_from_demux_result(
                demux_result=demux_result
            )

            bases_with_q30_percent = calculate_bases_with_q30_percent_from_demux_result(
                demux_result=demux_result
            )
            ...
            perfect_index_reads_percent = calculate_perfect_index_reads_percent_from_demux_result(
                demux_result=demux_result
            )
