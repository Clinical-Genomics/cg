...
def _qc_sample_info(outdata: dict, sampleinfo_raw: dict):
...
def _qc_metrics(outdata: dict, qcmetrics_raw: dict):
...
def _config(mip_config_raw: dict, outdata: dict):
...
def _add_genome_build(outdata: dict, sampleinfo_data: dict):
    outdata["genome_build"] = sampleinfo_data["genome_build"]


def _add_mip_version(outdata: dict, sampleinfo_data: dict):
    outdata["mip_version"] = sampleinfo_data["version"]


def _parse_qc_sample_info_file(sampleinfo_raw: dict):
    sampleinfo_data = parse_sampleinfo.parse_sampleinfo(sampleinfo_raw)
    return sampleinfo_data


def _add_sample_level_info_from_qc_metric_file(outdata: dict, qcmetrics_data: dict):
...
def _add_dropout_rates(outdata: dict, sample_data: dict):
    outdata["at_dropout"] = sample_data["at_dropout"]
    outdata["gc_dropout"] = sample_data["gc_dropout"]


def _add_insert_size_metrics(outdata: dict, sample_data: dict):
