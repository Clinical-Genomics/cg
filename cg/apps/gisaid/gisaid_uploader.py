...
parser.add_argument("-l", '--logfile',
                    help = "If set, all output will be logged into this file as JSON-object at the end of the run. If not set, all output will be printed to stdout and to './gisaid_uploader.log'")

parser.add_argument("-L", '--logfilenl',
                    help = "If set, all output will be logged into this file as newline separated JSON while running.'")
...
def parse_fasta(fn):
    seq = None
    header = None
    for row in open(fn, "rt", encoding = "utf-8-sig"):
        row = row.strip()
        if row:
            if row[0] == ">":
                if seq:
...
def handle(args):
    ...
    if args.logfilenl:
        if os.path.exists(args.logfilenl):
            os.remove(args.logfilenl)
    ...
    def log(code, *msg):
        if args.logfile:
            ...
            with open(args.logfilenl, "ta") as f:
