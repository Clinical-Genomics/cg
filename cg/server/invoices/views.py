from flask import flash, Blueprint, render_template, request, redirect, url_for, send_from_directory, session
...
def undo_invoice(invoice_id):
    ...
    recors = db.invoice_samples(invoice_id=invoice_id)
    invoice_obj.delete()
...
def index():
    """Show invoices."""
    if not logged_in(): return redirect(url_for('admin.index'))
...
def invoice(invoice_id):
    ...
    if not invoice_obj.price: final_price = api.total_price()
    else: final_price = invoice_obj.price
