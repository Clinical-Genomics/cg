...
class OrdersAPI(LimsHandler, StatusHandler):
                ...
                if self.osticket:
                    ...
                    for sample in data.get("samples"):
                        ...
                        if sample.get("internal_id"):
                            ...
                            existing_sample = self.status.sample(
                                sample.get("internal_id")
                            )
                            sample_customer = ""
                            if existing_sample.customer_id != data["customer"]:
                                sample_customer = (
                                    " from " + existing_sample.customer.internal_id
                                )
    ...
    def process_family_samples(self, data: dict) -> dict:
        ...
        new_samples = [
            sample for sample in data["samples"] if sample.get("internal_id") is None
        ]
        if new_samples:
        ...
        samples = [
            sample for family in status_data["families"] for sample in family["samples"]
        ]
        if lims_map:
    ...
    def update_application(self, ticket_number: int, families: List[models.Family]):
        ...
        for family_obj in families:
            ...
            if len(order_samples) >= 3:
                applications = [
                    sample_obj.application_version.application
                    for sample_obj in order_samples
                ]
                prep_categories = set(
                    application.prep_category for application in applications
                )
    ...
    def add_missing_reads(self, samples: List[models.Sample]):
        ...
        for sample_obj in samples:
            ...
            target_reads = (
                sample_obj.application_version.application.target_reads / 1000000
            )
            self.lims.update_sample(sample_obj.internal_id, target_reads=target_reads)

    @staticmethod
    def fill_in_sample_ids(
        samples: List[dict], lims_map: dict, id_key: str = "internal_id"
    ):
    ...
    def fill_in_sample_verified_organism(self, samples: List[dict]):
        for sample in samples:
            ...
            is_verified = (
                organism
                and organism.reference_genome == reference_genome
                and organism.verified
            )
    ...
    def _validate_customer_on_imported_samples(self, project, data):
        for sample in data.get("samples"):
            ...
            if sample.get("internal_id"):
                ...
                if (
                    existing_sample.customer.customer_group_id
                    != data_customer.customer_group_id
                ):
