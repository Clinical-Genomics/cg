...
def process_lims(lims_api: LimsAPI, data: dict, samples: List[dict]):
    ...
    samples_lims = to_lims(data["customer"], samples)
    project_name = data["ticket"] or data["name"]
    project_data = lims_api.lims.submit_project(project_name, samples_lims)
    lims_map = lims_api.lims.get_samples(projectlimsid=project_data["id"], map_ids=True)
