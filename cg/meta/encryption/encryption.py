...
from cg.exc import ChecksumFailedError, SequencingRunEncryptionError, SequencingRunError
...
class SequencingRunEncryptionAPI(EncryptionAPI):
    """Encryption functionality for sequencing runs."""
    ...
    @property
    def sequencing_run_encryption_dir(self) -> Path:
        return Path(self.encryption_dir, self.sequencing_run.full_name)

    @property
    def sequencing_run_encrypt_tmp_dir(self) -> Path:
        return Path(self.sequencing_run_encryption_dir, "tmp")

    @property
    def sequencing_run_encrypt_file_path_prefix(self) -> Path:
        return Path(self.sequencing_run_encryption_dir, self.sequencing_run.id)

    @property
    def pending_file_path(self) -> Path:
        return Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(FileExtensions.PENDING)
        )

    @property
    def complete_file_path(self) -> Path:
        return Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(FileExtensions.COMPLETE)
        )

    @property
    def encrypted_gpg_file_path(self) -> Path:
        return Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(
    ...
    def symmetric_passphrase_file_path(self) -> Path:
        return Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(FileExtensions.PASS_PHRASE)
        )

    @property
    def final_passphrase_file_path(self) -> Path:
        return Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(f".key{FileExtensions.GPG}")
        )

    def get_sequencing_run_symmetric_encryption_command(
    ...
    def get_sequencing_run_symmetric_decryption_command(
    ...
    def is_encryption_possible(self) -> bool | None:
        ...
        """
        if not self.sequencing_run.is_sequencing_run_ready():
    ...
    def make_tmp_encrypt_dir(self) -> str:
        return f"mkdir -p {self.sequencing_run_encrypt_tmp_dir}"

    def copy_sequencing_run_dir_to_tmp(self) -> str:
        return f"cp -r {self.sequencing_run.path} {self.sequencing_run_encrypt_tmp_dir}"

    def remove_tmp_encrypt_dir(self) -> str:
        return f"rm -rf {self.sequencing_run_encrypt_tmp_dir}"
    ...
    def encrypt_sequencing_run(
        ...
        commands: str = SEQUENCING_RUN_ENCRYPT_COMMANDS.format(
            ...
            copy_sequencing_run_dir_to_tmp=self.copy_sequencing_run_dir_to_tmp(),
            tar_encrypt_tmp_dir=self.tar_api.get_compress_cmd(
                input_path=self.sequencing_run_encrypt_tmp_dir
            ),
            ...
            sequencing_run_symmetric_encryption=self.get_sequencing_run_symmetric_encryption_command(
                output_file=self.encrypted_gpg_file_path,
                passphrase_file_path=self.symmetric_passphrase_file_path,
            ),
            sequencing_run_symmetric_decryption=self.get_sequencing_run_symmetric_decryption_command(
        ...
        sbatch_parameters = Sbatch(
            ...
            job_name="_".join([self.sequencing_run.id, "sequencing_run_encryption"]),
            log_dir=self.sequencing_run_encryption_dir.as_posix(),
        ...
        sbatch_path = Path(
            self.sequencing_run_encrypt_file_path_prefix.with_suffix(FileExtensions.SBATCH)
        )
        sbatch_number: int = self.slurm_api.submit_sbatch(
            sbatch_content=sbatch_content, sbatch_path=sbatch_path
        )
        LOG.info(f"Sequencing run encryption running as job {sbatch_number}")
    ...
    def start_encryption(self) -> None:
        ...
        self.sequencing_run_encryption_dir.mkdir(exist_ok=True, parents=True)
        self.create_pending_file(pending_path=self.pending_file_path)
        self.encrypt_sequencing_run()
