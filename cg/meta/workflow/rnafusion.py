...
class RnafusionAnalysisAPI(AnalysisAPI):
    ...
    def write_samplesheet(self, case_id: str, strandedness = 'reverse') -> None:
        ...
        for link in case_obj.links:
            ...
            samplesheet = pd.DataFrame(list(zip(fastq_R1, fastq_R2)),columns =['fastq_1','fastq_2'])
            ...
            samplesheet.to_csv(filename, index=False, columns = ['sample', 'fastq_1', 'fastq_2','strandedness'])
    ...
    def run_analysis(
        ...
        options = self.__build_command_str(self.get_verified_arguments(
            case_id=case_id,
            work_dir=work_dir,
            resume=resume,
            profile=profile,
        ...
        nextflow_options = self.__build_command_str(self.get_verified_arguments_nextflow(
            case_id=case_id,
            bg=bg,
            log=log,
        ))
        command = ["run", self.nfcore_pipeline_path]
        parameters = nextflow_options + ["-bg", "-q"] + command + options \
        + [" > "+ str(self.get_case_path(case_id)) + "/" + case_id + "-stdout.log 2> "
        + str(self.get_case_path(case_id)) + "/" + case_id + "-stdout.err  < /dev/null & "]
