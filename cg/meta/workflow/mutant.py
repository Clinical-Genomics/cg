...
class MutantAnalysisAPI(AnalysisAPI):
    ...
    def get_cases_to_store(self) -> list[Case]:
        ...
        # Gather running cases with deliverables
        cases = case in self.status_db.get_running_cases_in_pipeline(pipeline=self.pipeline) if Path(self.get_deliverables_file_path(case_id=case.internal_id)).exists()

        # send each case to qc check
        for case in cases:
            qc_check(case)

        # Then return them to be stored
        return [
            case
            for case in cases
        ]
