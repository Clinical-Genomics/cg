...
def has_valid_total_reads(
    ...
    if external_negative_control:
        return external_negative_control_sample_has_valid_reads(reads=sample.reads)

    if internal_negative_control:
        return internal_negative_control_sample_has_valid_reads(reads=sample.reads)

    return sample_has_enough_reads(sample=sample)


def external_negative_control_sample_has_valid_reads(reads: int) -> bool:
    return reads < MutantQC.EXTERNAL_NEGATIVE_CONTROL_READS_THRESHOLD


def internal_negative_control_sample_has_valid_reads(reads: int) -> bool:
