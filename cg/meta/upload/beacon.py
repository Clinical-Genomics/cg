...
class UploadBeaconApi:
    ...
    def upload(
        ...
        hk_vcf = self.housekeeper.files(
            version=hk_version.id, tags=["vcf-snv-clinical"]
        ).first()
        ...
        if sample_ids:
            ...
            if panel[0] == "None":
                LOG.info(
                    "Panel was set to 'None', so all variants are going to be uploaded."
                )
                path_to_panel = None
                status_msg += "|" + str(path_to_panel)
            else:
                print(
                    "Generating variants filter based on ", len(panel), " gene panels"
                )
                bed_lines = self.scout.export_panels(panel, None)
                temp_panel = NamedTemporaryFile(
                    "w+t", suffix="_chiara." + ",".join(panel)
                )
                temp_panel.write("\n".join(bed_lines))
                ...
                with open(temp_panel.name, "r") as panel_lines:
                    while n_panels:
                        for line in panel_lines:
                            if line.startswith(
                                "##gene_panel="
                            ):  # header with specifics of 1 panel
    ...
    def create_bed_panels(self, list_of_panels: list):
        ...
        if scout_panels.sort() == list_of_panels.sort():
            LOG.info(
                "Panels retrieved in scout corespond to those used for beacon upload."
            )
            return temp_panel
        else:
            return None
        ...
        if item_type == "family":  # remove vars for all affected samples in a family:
            ...
            if samples_to_remove:
                ...
                for sample in samples_to_remove:
                    ...
                    LOG.info(
                        "######## Processing sample %s ########", sample.internal_id
                    )
                ...
                LOG.warn(
                    "Could't find any affected sample in beacon for family %s!", item_id
                )
            ...
            if sample_obj:
                ...
                LOG.info(
                    "######## Processing sample %s ########", sample_obj.internal_id
                )
                ...
                LOG.critical(
                    "Couldn't find a sample named '%s' in cg database!", item_id
                )
