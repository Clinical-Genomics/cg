...
class UploadBeaconApi():
    ...
    def upload(self, family_id: str, panel: list, dataset: str = 'clinicalgenomics', outfile: str = None, customer: str = None, qual: int = 20, reference: str = "grch37"):
        ...
        if hk_vcf is None:
            ...
            hk_vcf = self.housekeeper.files(version=hk_version.id, tags=[
                                            'vcf-clinical-bin']).first()
                ...
                if sample_obj.internal_id in vcf_samples:
                    ...
                    if not beacon_info[3] == 'None':
                        ...
                        if temp_panel:
                            ...
                            LOG.info("Variants removed for sample %s: %s",
                                     sample_obj.internal_id, results)
                    ...
                    LOG.warn("sample %s is not contained in the annotated vcf file!",
