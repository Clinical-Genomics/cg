...
class UploadObservationsAPI:
    ...
    def __init__(
        self, status_api: Store, hk_api: hk.HousekeeperAPI, loqus_api: loqus.LoqusdbAPI
    ):
    ...
    def get_input(self, analysis_obj: models.Analysis) -> dict:
        ...
        hk_vcf = self.housekeeper.files(
            version=hk_version.id, tags=["vcf-snv-research"]
        ).first()
        hk_snv_gbcf = self.housekeeper.files(
            version=hk_version.id, tags=["snv-gbcf"]
        ).first()
        hk_pedigree = self.housekeeper.files(
            version=hk_version.id, tags=["pedigree"]
        ).first()
        ...
        if self.loqusdb.analysis_type == "wgs":
            ...
            if hk_sv_vcf is None:
                raise FileNotFoundError(
                    "No file with vcf-sv-research tag in housekeeper"
                )
        ...
        for link in analysis_obj.family.links:
            if link.sample.loqusdb_id:
                raise DuplicateRecordError(
                    f"{link.sample.internal_id} already in LoqusDB"
                )
