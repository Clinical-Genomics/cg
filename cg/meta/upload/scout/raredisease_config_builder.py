...
class RarediseaseConfigBuilder(ScoutConfigBuilder):
    ...
    def build_load_config(
        ...
        self.get_sample_information(
            load_config=load_config, analysis_obj=analysis_obj, hk_version_obj=hk_version_obj
        )
        ...
        load_config.rank_model_version = self.get_rank_model_version(
            variant_type=Variants.SNV, hk_version_obj=hk_version_obj
        )
        load_config.sv_rank_model_version = self.get_rank_model_version(
            variant_type=Variants.SV, hk_version_obj=hk_version_obj
        )
        return load_config

    def get_rank_model_version(self, variant_type: Variants, hk_version_obj: Version) -> str:
        """
        ...
        hk_manifest_file: File = self.get_file_from_hk(
            {NFAnalysisTags.MANIFEST}, hk_version_obj=hk_version_obj
        )
    ...
    def load_custom_image_sample(
        self, load_config: RarediseaseLoadConfig, analysis_obj: Analysis, hk_version_obj: Version
    ) -> None:
        """Build custom images config."""
        LOG.info("Adding custom images")

        eklipse_images: list = []
        for sample in analysis_obj.case.samples:
    ...
    def include_case_files(
        self, load_config: RarediseaseLoadConfig, hk_version_obj: Version
    ) -> None:
        ...
        for scout_key in RAREDISEASE_CASE_TAGS.keys():
            self._include_case_file(
                load_config=load_config, scout_key=scout_key, hk_version_obj=hk_version_obj
            )

    def _include_case_file(
        self, load_config: RarediseaseLoadConfig, scout_key: str, hk_version_obj: Version
    ) -> None:
        ...
        file_path = self.get_file_from_hk(
            getattr(self.case_tags, scout_key), hk_version_obj=hk_version_obj
        )
        setattr(load_config, scout_key, file_path)

    def include_sample_files(
        self, config_sample: ScoutRarediseaseIndividual, hk_version_obj: Version
    ) -> None:
        ...
        config_sample.d4_file = self.get_sample_file(
            hk_tags=self.sample_tags.d4_file, sample_id=sample_id, hk_version_obj=hk_version_obj
        )
