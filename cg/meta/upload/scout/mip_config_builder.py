...
class MipConfigBuilder(ScoutConfigBuilder):
    ...
    def build_load_config(
        ...
        load_config.gene_panels = (
            self.mip_analysis_api.get_aggregated_panels(
                customer_id=analysis_obj.case.customer.internal_id,
                default_panels=set(analysis_obj.case.panels),
            )
            or None
        )
    ...
    def include_case_files(self, load_config: MipLoadConfig, hk_version_obj: Version) -> None:
        ...
        load_config.peddy_sex = self.get_file_from_hk(
            self.case_tags.peddy_sex, hk_version_obj=hk_version_obj
        )
        load_config.smn_tsv = self.get_file_from_hk(
            self.case_tags.smn_tsv, hk_version_obj=hk_version_obj
        )
        load_config.vcf_mei = self.get_file_from_hk(
            self.case_tags.vcf_mei, hk_version_obj=hk_version_obj
        )
        load_config.vcf_mei_research = self.get_file_from_hk(
            self.case_tags.vcf_mei_research, hk_version_obj=hk_version_obj
        )
        load_config.vcf_snv = self.get_file_from_hk(
            self.case_tags.snv_vcf, hk_version_obj=hk_version_obj
        )
        load_config.vcf_snv_research = self.get_file_from_hk(
            self.case_tags.snv_research_vcf, hk_version_obj=hk_version_obj
        )
        load_config.vcf_str = self.get_file_from_hk(
            self.case_tags.vcf_str, hk_version_obj=hk_version_obj
        )
        load_config.vcf_sv = self.get_file_from_hk(
            self.case_tags.sv_vcf, hk_version_obj=hk_version_obj
        )
        load_config.vcf_sv_research = self.get_file_from_hk(
            self.case_tags.sv_research_vcf, hk_version_obj=hk_version_obj
        )
        self.include_multiqc_report(load_config=load_config, hk_version_obj=hk_version_obj)

    def include_sample_files(
        self, config_sample: ScoutMipIndividual, hk_version_obj: Version
    ) -> None:
        ...
        config_sample.mt_bam = self.get_sample_file(
            hk_tags=self.sample_tags.mt_bam, sample_id=sample_id, hk_version_obj=hk_version_obj
        )
        ...
        config_sample.reviewer.catalog = self.get_file_from_hk(
            hk_tags=self.case_tags.str_catalog, hk_version_obj=hk_version_obj
        )
