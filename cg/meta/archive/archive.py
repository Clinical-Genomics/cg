...
class ArchiveAPI:
    """Class handling the archival of sample SPRING files."""

    def __int__(self, ddn_dataflow_api: DDNDataFlowApi, housekeeper_api: HousekeeperAPI):
    ...
    def update_archive_tasks(self):
        unfinished_tasks: List[int] = self.housekeeper_api.get_unfinished_tasks()
        for task_id in unfinished_tasks:
            if self.ddn_api.is_task_completed(task_id=task_id):
                self.housekeeper_api.set_archive_time_stamps(task_id)
