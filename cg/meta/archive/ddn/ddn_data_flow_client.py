...
from cg.meta.archive.ddn.utils import (
    get_metadata,
    get_request_log,
)
...
class DDNDataFlowClient(ArchiveHandler):
    ...
    def _get_auth_token(self) -> AuthToken:
        ...
        response: Response = APIRequest.api_request_from_content(
            ...
            url=urljoin(
                base=self.url,
                url=DataflowEndpoints.GET_AUTH_TOKEN,
            ),
    ...
    def _get_refreshed_auth_token(
        ...
        response: Response = APIRequest.api_request_from_content(
            ...
            url=urljoin(
                base=self.url,
                url=DataflowEndpoints.REFRESH_AUTH_TOKEN,
            ),
            headers=self.headers,
    ...
    def archive_file(self, file_and_sample: FileAndSample) -> int:
        ...
        archival_request: TransferPayload = self.create_transfer_request(
            miria_file_data=miria_file_data,
    ...
    def convert_into_transfer_data(
        ...
        return [
            MiriaObject.create_from_file_and_sample(
                file=file_and_sample.file,
                sample=file_and_sample.sample,
    ...
    def _get_job_status(self, headers: dict, job_id: int) -> GetJobStatusResponse:
        ...
        url: str = urljoin(
            self.url,
            DataflowEndpoints.GET_JOB_STATUS + str(job_id),
        )
        response: Response = APIRequest.api_request_from_content(
            api_method=APIMethods.POST,
            url=url,
