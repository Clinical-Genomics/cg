...
class DemuxedFlowcell:
    ...
    def __init__(
        ...
        self.flowcell_name: str = self.flowcell_path.name
        self.split_flowcell_name: List[str] = re.split("_|\.|-", self.flowcell_name)
        self.flowcell_identifier: str = self.split_flowcell_name[3]
        self.flowcell_id: str = self.flowcell_identifier[1:]
        self.status_db: Store = status_db
        self.hk: HousekeeperAPI = housekeeper_api
        self.is_correctly_named: bool = True
        self.flowcell_exists_in_status_db: bool = True
        self.flowcell_status_is_on_disk: bool = True
        self.fastq_files_exist_in_housekeeper: bool = True
        self.fastq_files_exist_on_disk: bool = True
        self.passed_check: bool = True

    def check_flowcell_naming(self) -> None:
        """checks if the flowcell directory is correctly named: it should end with the flowcell
        id"""
        self.is_correctly_named: bool = self.split_flowcell_name[-1] == self.flowcell_identifier
        if not self.is_correctly_named:
            LOG.warning("Flowcell name not correctly named!: %s", self.flowcell_path)

    def check_flowcell_exists_in_status_db(self) -> None:
        """checks if flowcell exists in statusdb"""
        self.flowcell_exists_in_status_db: bool = (
            self.status_db.flowcell(self.flowcell_id) is not None
        )
        if not self.flowcell_exists_in_status_db:
            LOG.warning("Flowcell %s does not exist in statusdb!", self.flowcell_id)

    @property
    def flowcell_status(self) -> Optional[str]:
        """Returns the flowcell status in statusdb"""
        try:
            return self.status_db.flowcell(self.flowcell_id).status
        except AttributeError:
            return

    def check_flowcell_status_in_status_db(self) -> None:
        """checks if the flowcell has the status "ondisk in statusdb"""
        self.flowcell_status_is_on_disk: bool = self.flowcell_status == "ondisk"
        if not self.flowcell_status_is_on_disk:
            LOG.warning(
    ...
    def check_fastq_files_in_housekeeper(self) -> None:
        ...
        self.fastq_files_exist_in_housekeeper = (
            self.hk.files(tags=[self.flowcell_id, "fastq"]).count() > 0
        )
        if not self.fastq_files_exist_in_housekeeper:
            LOG.warning(
                "Flowcell %s does not have any fastq files in Housekeeper!", self.flowcell_id
            )
    ...
    def check_fastq_files_on_disk(self):
        ...
        if self.fastq_files_exist_in_housekeeper:
            fastq_files = self.hk.files(tags=[self.flowcell_id, "fastq"])
            ...
            if not self.fastq_files_exist_on_disk:
                LOG.warning(
                    "Flowcell %s has fastq files in Housekeeper, but not on disk!", self.flowcell_id
                )
    ...
    def check_all_checks(self):
        ...
        self.passed_check = all(
            [
                ...
                self.flowcell_exists_in_status_db,
                self.flowcell_status_is_on_disk,
                self.fastq_files_exist_in_housekeeper,
    ...
    def remove_files_from_housekeeper(self):
        ...
        sample_sheets = self.hk.files(tags=[self.flowcell_id, "samplesheet"])
    ...
    def remove_from_demultiplexed_runs(self):
        ...
        shutil.rmtree(self.flowcell_path, ignore_errors=True)
        if self.flowcell_status_is_on_disk and self.is_correctly_named:
            self.status_db.flowcell(self.flowcell_id).status = "removed"
        if self.fastq_files_exist_in_housekeeper:
