...
class CleanRetrievedSpringFilesAPI:
    ...
    def _unlink_files(self, files_to_remove: list[File]) -> None:
    ...
    def clean_retrieved_spring_files(self):
        """Removes Spring files retrieved more than 7 days ago from Hasta, and resets retrieval data in Housekeeper."""
        files_to_remove: list[File] = self._get_files_to_remove()
        self._unlink_files(files_to_remove=files_to_remove)
        if not self.dry_run:
