...
def get_lane_from_sample_fastq(
...
def get_sample_fastqs_from_flow_cell(
    ...
    for pattern in [
        ...
        sample_fastqs: list[Path] = get_files_matching_pattern(
            directory=flow_cell_directory,
            pattern=pattern,
        )
        valid_sample_fastqs: list[Path] = get_valid_sample_fastqs(
            fastq_paths=sample_fastqs,
            sample_internal_id=sample_internal_id,
        )
...
def create_delivery_file_in_flow_cell_directory(
    flow_cell_directory: Path,
) -> None:
    Path(
        flow_cell_directory,
...
def get_sample_sheet_path_from_flow_cell_dir(
    ...
    return get_file_in_directory(
        directory=flow_cell_directory,
        file_name=sample_sheet_file_name,
    )
...
def add_flow_cell_name_to_fastq_file_path(fastq_file_path: Path, flow_cell_name: str) -> Path:
    ...
    if is_pattern_in_file_path_name(
        file_path=fastq_file_path,
        pattern=flow_cell_name,
    ):
        LOG.debug(
    ...
    return Path(
        fastq_file_path.parent,
        f"{flow_cell_name}_{fastq_file_path.name}",
    )
...
def rename_fastq_file_if_needed(fastq_file_path: Path, flow_cell_name: str) -> Path:
    ...
    if fastq_file_path != renamed_fastq_file_path:
        rename_file(
            file_path=fastq_file_path,
            renamed_file_path=renamed_fastq_file_path,
        )
...
def is_file_relevant_for_demultiplexing(
    file: Path,
) -> bool:
...
def get_existing_manifest_file(
...
def are_all_files_synced(
    files_at_source: list[Path],
...
def is_syncing_complete(source_directory: Path, target_directory: Path) -> bool:
    ...
    return are_all_files_synced(
        files_at_source=files_at_source,
        target_directory=target_directory,
    )


def get_flow_cell_id(
    flow_cell_dir_name: str,
) -> str:
...
def is_manifest_file_required(
    ...
    return (
        not any(
            (
                illumina_manifest_file.exists(),
                custom_manifest_file.exists(),
            )
        )
...
def is_flow_cell_sync_confirmed(
    target_flow_cell_dir: Path,
) -> bool:
    return Path(
        target_flow_cell_dir,
        DemultiplexingDirsAndFiles.COPY_COMPLETE,
    ).exists()


def get_nanopore_summary_file(
    flow_cell_directory: Path,
) -> bool | None:
...
def is_nanopore_sequencing_complete(
    flow_cell_directory: Path,
) -> bool:
