...
def is_demultiplexing_complete(
...
def is_flow_cell_ready_for_delivery(
    flow_cell_directory: Path,
) -> bool:
    return Path(
        flow_cell_directory,
...
def validate_sample_sheet_exists(
...
def validate_demultiplexing_complete(
...
def validate_flow_cell_has_fastq_files(
    flow_cell: FlowCellDirectoryData,
) -> None:
...
def is_flow_cell_ready_for_postprocessing(
    ...
    validate_flow_cell_delivery_status(
        flow_cell_output_directory=flow_cell_output_directory,
        force=force,
    )
