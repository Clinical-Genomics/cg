...
def filter_orders_by_closed(orders: Query, closed: bool | None, **kwargs) -> Query:
    return orders.filter(Order.is_open != closed) if closed is not None else orders
...
class OrderFilter(Enum):
    ...
    BY_CLOSED: Callable = filter_orders_by_closed
