...
def filter_by_device_internal_id_search(runs: Query, id_search: str, **kwargs) -> Query:
    """Filter sequencing runs by flow cell id enquiry."""
    joined_query: Query = runs.join(IlluminaFlowCell)
    return joined_query.filter(IlluminaFlowCell.internal_id.contains(id_search))
...
class IlluminaSequencingRunFilter(Enum):
    ...
    BY_DEVICE_INTERNAL_ID_SEARCH: callable = filter_by_device_internal_id_search
