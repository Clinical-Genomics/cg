...
def get_bed_version_by_filename(bed_versions: Query, bed_version_filename: str, **kwargs) -> Query:
    """Return beds by file name."""
    return bed_versions.filter(BedVersion.filename == bed_version_filename)
...
class BedVersionFilter(Enum):
    ...
    FILTER_BY_FILE_NAME: Callable = get_bed_version_by_filename
...
def apply_bed_version_filter(
    ...
    bed_version_filename: Optional[str] = None,
