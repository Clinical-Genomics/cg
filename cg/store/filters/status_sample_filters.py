from enum import Enum
from typing import Any, Callable

from sqlalchemy import or_
from sqlalchemy.orm import Query

from cg.constants.constants import SampleType
from cg.store.models import Customer, Sample


def get_samples_by_internal_id(internal_id: str, samples: Query, **kwargs) -> Query:
    """Return sample by internal id."""
    return samples.filter(Sample.internal_id == internal_id)


def get_samples_by_name(name: str, samples: Query, **kwargs) -> Query:
    """Return sample with sample name."""
    return samples.filter(Sample.name == name)


def get_samples_with_type(samples: Query, tissue_type: SampleType, **kwargs) -> Query:
    """Return samples with sample type."""
    is_tumour: bool = tissue_type == SampleType.TUMOR
    return samples.filter(Sample.is_tumour == is_tumour)


def get_samples_with_loqusdb_id(samples: Query, **kwargs) -> Query:
    """Return samples with a loqusdb ID."""
    return samples.filter(Sample.loqusdb_id.isnot(None))


def get_samples_without_loqusdb_id(samples: Query, **kwargs) -> Query:
    """Return samples without a loqusdb ID."""
    return samples.filter(Sample.loqusdb_id.is_(None))


def get_samples_by_entry_id(entry_id: int, samples: Query, **kwargs) -> Query:
    """Return sample with entry id."""
    return samples.filter_by(id=entry_id)


def get_samples_is_delivered(samples: Query, **kwargs) -> Query:
    """Return delivered samples."""
    return samples.filter(Sample.delivered_at.isnot(None))


def get_samples_is_not_delivered(samples: Query, **kwargs) -> Query:
    """Return samples that are not delivered."""
    return samples.filter(Sample.delivered_at.is_(None))


def get_samples_by_invoice_id(samples: Query, invoice_id: int, **kwargs) -> Query:
    """Return samples by invoice_id"""
    return samples.filter(Sample.invoice_id == invoice_id)


def get_samples_without_invoice_id(samples: Query, **kwargs) -> Query:
    """Return samples that are not attached to an invoice."""
    return samples.filter(Sample.invoice_id.is_(None))


def get_samples_is_not_down_sampled(samples: Query, **kwargs) -> Query:
    """Return samples that are not down sampled."""
    return samples.filter(Sample.downsampled_to.is_(None))


def get_samples_is_sequenced(samples: Query, **kwargs) -> Query:
    """Return samples that are sequenced."""
    return samples.filter(Sample.last_sequenced_at.isnot(None))


def get_samples_is_not_sequenced(samples: Query, **kwargs) -> Query:
    """Return samples that are not sequenced."""
    return samples.filter(Sample.last_sequenced_at.is_(None))


def get_samples_do_invoice(samples: Query, **kwargs) -> Query:
    """Return samples that should be invoiced."""
    return samples.filter(Sample.no_invoice.is_(False))


def get_samples_by_entry_customer_ids(
    samples: Query, customer_entry_ids: list[int], **kwargs
) -> Query:
    """Return samples by customer id."""
    return samples.filter(Sample.customer_id.in_(customer_entry_ids))


def get_samples_is_received(samples: Query, **kwargs) -> Query:
    """Return samples that are received."""
    return samples.filter(Sample.received_at.isnot(None))


def get_samples_is_not_received(samples: Query, **kwargs) -> Query:
    """Return samples that are not received."""
    return samples.filter(Sample.received_at.is_(None))


def get_samples_is_prepared(samples: Query, **kwargs) -> Query:
    """Return samples that are prepared."""
    return samples.filter(Sample.prepared_at.isnot(None))


def get_samples_is_not_prepared(samples: Query, **kwargs) -> Query:
    """Return samples that are not prepared."""
    return samples.filter(Sample.prepared_at.is_(None))


def get_samples_by_subject_id(samples: Query, subject_id: str, **kwargs) -> Query:
    """Return samples by subject id."""
    return samples.filter(Sample.subject_id == subject_id)


def get_samples_is_tumour(samples: Query, **kwargs) -> Query:
    """Return samples that are tumour."""
    return samples.filter(Sample.is_tumour.is_(True))


def get_samples_is_not_tumour(samples: Query, **kwargs) -> Query:
    """Return samples that are not tumour."""
    return samples.filter(Sample.is_tumour.is_(False))


def get_samples_by_internal_id_pattern(samples: Query, internal_id_pattern: str, **kwargs) -> Query:
    """Return samples matching the internal id pattern."""
    return samples.filter(Sample.internal_id.like(f"%{internal_id_pattern}%"))


def get_samples_by_internal_id_or_name_search(
    samples: Query, search_pattern: str, **kwargs
) -> Query:
    """Return samples matching the internal id or name search."""
    return samples.filter(
        or_(
            Sample.name.like(f"%{search_pattern}%"),
            Sample.internal_id.like(f"%{search_pattern}%"),
        )
    )


def get_samples_by_customer(samples: Query, customer: Customer, **kwargs) -> Query:
    """Return samples by customer."""
    return samples.filter(Sample.customer == customer)


def order_samples_by_created_at_desc(samples: Query, **kwargs) -> Query:
    """Return samples ordered by created_at descending."""
    return samples.order_by(Sample.created_at.desc())


def get_samples_by_identifier_name_and_value(
    samples: Query, identifier_name: str, identifier_value: Any, **kwargs
) -> Query:
    """Filters the sample query by the given identifier name and value."""
    return samples.filter(getattr(Sample, identifier_name) == identifier_value)


def apply_sample_filter(
    filter_functions: list[Callable],
    samples: Query,
    entry_id: int | None = None,
    internal_id: str | None = None,
    tissue_type: SampleType | None = None,
    data_analysis: str | None = None,
    invoice_id: int | None = None,
    customer_entry_ids: list[int] | None = None,
    subject_id: str | None = None,
    name: str | None = None,
    customer: Customer | None = None,
    name_pattern: str | None = None,
    internal_id_pattern: str | None = None,
    search_pattern: str | None = None,
    identifier_name: str = None,
    identifier_value: Any = None,
) -> Query:
    """Apply filtering functions to the sample queries and return filtered results."""

    for filter_function in filter_functions:
        samples: Query = filter_function(
            samples=samples,
            entry_id=entry_id,
            internal_id=internal_id,
            tissue_type=tissue_type,
            data_analysis=data_analysis,
            invoice_id=invoice_id,
            customer_entry_ids=customer_entry_ids,
            subject_id=subject_id,
            name=name,
            customer=customer,
            name_pattern=name_pattern,
            internal_id_pattern=internal_id_pattern,
            search_pattern=search_pattern,
            identifier_name=identifier_name,
            identifier_value=identifier_value,
        )
    return samples


class SampleFilter(Enum):
    """Define Sample filter functions."""

    GET_BY_CUSTOMER: Callable = get_samples_by_customer
    GET_BY_CUSTOMER_ENTRY_IDS: Callable = get_samples_by_entry_customer_ids
    GET_BY_ENTRY_ID: Callable = get_samples_by_entry_id
    GET_BY_IDENTIFIER_NAME_AND_VALUE: Callable = get_samples_by_identifier_name_and_value
    GET_BY_INTERNAL_ID: Callable = get_samples_by_internal_id
    GET_BY_INTERNAL_ID_OR_NAME_SEARCH: Callable = get_samples_by_internal_id_or_name_search
    GET_BY_INTERNAL_ID_PATTERN: Callable = get_samples_by_internal_id_pattern
    GET_BY_INVOICE_ID: Callable = get_samples_by_invoice_id
    GET_BY_SAMPLE_NAME: Callable = get_samples_by_name
    GET_BY_SUBJECT_ID: Callable = get_samples_by_subject_id
    GET_DO_INVOICE: Callable = get_samples_do_invoice
    GET_HAS_NO_INVOICE_ID: Callable = get_samples_without_invoice_id
    GET_IS_DELIVERED: Callable = get_samples_is_delivered
    GET_IS_NOT_DELIVERED: Callable = get_samples_is_not_delivered
    GET_IS_NOT_DOWN_SAMPLED: Callable = get_samples_is_not_down_sampled
    GET_IS_PREPARED: Callable = get_samples_is_prepared
    GET_IS_NOT_PREPARED: Callable = get_samples_is_not_prepared
    GET_IS_RECEIVED: Callable = get_samples_is_received
    GET_IS_NOT_RECEIVED: Callable = get_samples_is_not_received
    GET_IS_SEQUENCED: Callable = get_samples_is_sequenced
    GET_IS_NOT_SEQUENCED: Callable = get_samples_is_not_sequenced
    GET_IS_TUMOUR: Callable = get_samples_is_tumour
    GET_IS_NOT_TUMOUR: Callable = get_samples_is_not_tumour
    GET_WITH_LOQUSDB_ID: Callable = get_samples_with_loqusdb_id
    GET_WITHOUT_LOQUSDB_ID: Callable = get_samples_without_loqusdb_id
    GET_WITH_TYPE: Callable = get_samples_with_type
    ORDER_BY_CREATED_AT_DESC: Callable = order_samples_by_created_at_desc
