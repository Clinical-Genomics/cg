...
class CreateMixin(ReadHandler):
    ...
    def create_pacbio_sequencing_run(
        ...
        if (
            ...
            .filter(PacbioSequencingRun.internal_id == pacbio_sequencing_run_dto.internal_id)
            .first()
        ):
            raise PacbioSequencingRunAlreadyExistsError(
                message=f"{pacbio_sequencing_run_dto.internal_id} already exists."
            )
        else:
            LOG.debug(f"Creating Pacbio Sequencing Run for {pacbio_sequencing_run_dto.internal_id}")
            sequencing_run = PacbioSequencingRun(
                instrument_name=pacbio_sequencing_run_dto.instrument_name,
                internal_id=pacbio_sequencing_run_dto.internal_id,
            )
