...
class StatusHandler(BaseHandler):
    ...
    def cases_to_mip_analyze(self, limit: int = 50):
        ...
        families_q = (
            ...
            .filter(
                or_(models.Sample.is_external, models.Sample.sequenced_at.isnot(None))
            )
            .filter(
                or_(
    ...
    def cases(
        ...
        if internal_id:
            families_q = families_q.filter(
                models.Family.internal_id.like("%" + internal_id + "%")
            )
        ...
        if customer_id or exclude_customer_id:
            ...
            if customer_id:
                families_q = families_q.filter(
                    models.Customer.internal_id == customer_id
                )

            if exclude_customer_id:
                families_q = families_q.filter(
                    models.Customer.internal_id != exclude_customer_id
                )

        # sample filters
        if data_analysis or sample_id:
            families_q = families_q.join(
                models.Family.links, models.FamilySample.sample
            )
            if data_analysis:
                families_q = families_q.filter(
            ...
            if sample_id:
                families_q = families_q.filter(
                    models.Sample.internal_id.like(sample_id)
                )
        ...
        for record in families_q:
            ...
            if total_samples > 0:
                ...
                samples_to_invoice = total_samples - len(
                    [
                        link.sample.no_invoice
                        for link in record.links
                        if link.sample.no_invoice
                    ]
                )
            ...
            if record.analyses and not analysis_in_progress:
                ...
                analysis_delivery_reported_at = record.analyses[
                    0
                ...
                analysis_delivery_reported_bool = (
                    analysis_delivery_reported_at is not None
                )
    ...
    def _all_samples_have_sequence_data(links: List[models.FamilySample]) -> bool:
        ...
        return all(
            (link.sample.sequenced_at or link.sample.is_external) for link in links
        )
    ...
    def analyses_to_deliver(self):
        ...
        records = (
            self.Analysis.query.join(
                models.Family, models.Family.links, models.FamilySample.sample
            )
            .filter(
            ...
            .filter(
                ...
                or_(
                    ...
                    and_(
                        ...
                        models.Analysis.delivery_report_created_at
                        < models.Sample.delivered_at,
                    ),
                ),
            )
            .order_by(models.Analysis.uploaded_at.desc())
    ...
    def microbial_samples_to_invoice(self, customer: models.Customer = None):
        ...
        customers_to_invoice = [
            record.microbial_order.customer for record in records.all()
        ]
    ...
    def samples_to_invoice(self, customer: models.Customer = None):
        ...
        customers_to_invoice = list(set(customers_to_invoice))
        records = (
            records.filter(models.Sample.customer == customer) if customer else records
        )
        return records, customers_to_invoice
    ...
    def pools_to_invoice(self, customer: models.Customer = None):
        ...
        customers_to_invoice = list(set(customers_to_invoice))
        records = (
            records.filter(models.Pool.customer_id == customer.id)
            if customer
            else records
        )
        return records, customers_to_invoice
    ...
    def _calculate_estimated_turnaround_time(
        ...
        if samples_received_at and samples_delivered_at:
            return self._calculate_date_delta(
                None, samples_received_at, samples_delivered_at
            )
        ...
        if is_rerun:
            o_a = self._calculate_date_delta(
                1, analysis_ordered_at, analysis_completed_at
            )
            return o_a + a_u
        ...
        if external_case_bool:
            if analysis_ordered_at and analysis_uploaded_at:
                return self._calculate_date_delta(
                    None, analysis_ordered_at, analysis_uploaded_at
                )
    ...
    def _get_max_tat(links):
        ...
        for link in links:
            if link.sample.application_version.application.turnaround_time:
                max_tat = max(
                    0, link.sample.application_version.application.turnaround_time
                )
        return max_tat
