    ...
    def samples_per_month(self, year):
        ...
        for cust_priority, results in groupby(
            ...
            data = {
                "name": cust_priority,
                "results": {
                    month: counts.get(month) or None for month in MONTHS.values()
                },
            }
            yield data
        ...
        query = (
            self.session.query(
                ...
                (
                    func.avg(
                        func.datediff(
                            models.Sample.delivered_at, models.Sample.received_at
                        )
                    ).label("average")
                ),
            )
            ...
            .group_by(
                models.Application.category, func.month(models.Sample.received_at)
            )
        )
        for category, results in groupby(query, key=lambda result: result.category):
            averages = {
                MONTHS[result.month_no]: float(result.average)
                if result.average
                else None
            ...
            yield {
                "name": category,
                "results": {
                    month: averages.get(month) or None for month in MONTHS.values()
                },
            }
        ...
        query = (
            self.session.query(
                ...
                (
                    func.avg(
                        func.datediff(
                            models.Sample.prepared_at, models.Sample.received_at
                        )
                    ).label("average")
                ),
            )
    ...
    def sequenced_to_delivered(self, year):
        ...
        query = (
            self.session.query(
                ...
                func.avg(
                    func.datediff(
                        models.Sample.delivered_at, models.Sample.sequenced_at
                    )
                ).label("average"),
            )
            .join(
    ...
    def delivered_to_invoiced(self, year):
        ...
        query = (
            self.session.query(
                ...
                func.avg(
                    func.datediff(
                        models.Invoice.invoiced_at, models.Sample.delivered_at
                    )
                ).label("average"),
            )
            .join(
