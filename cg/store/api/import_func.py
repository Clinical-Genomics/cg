...
def import_application_versions(store, excel_path, sign, dry_run, skip_missing):
    ...
    for raw_version in raw_versions:
        ...
        if latest_version and versions_are_same(
            latest_version, raw_version, workbook.datemode
        ):
            logging.info(
                "skipping redundant application version for app tag %s", app_tag
            )
            continue

        logging.info(
            "adding new application version to transaction for app tag %s", app_tag
        )
    ...
    if not dry_run:
        logging.info(
            "all application versions successfully added to transaction, committing "
            "transaction"
        )
        store.commit()
    else:
...
def import_applications(store, excel_path, sign, dry_run, sheet_name=None):
    ...
    if not dry_run:
        logging.info(
            "all applications successfully added to transaction, committing transaction"
        )
        store.commit()
    else:
        logging.error("Dry-run, rolling back transaction.")
...
def versions_are_same(
    version_obj: models.ApplicationVersion, raw_version: dict, datemode: int
):
...
def applications_are_same(application_obj: models.Application, raw_application: dict):
    ...
    return application_obj and application_obj.tag == _get_tag_from_raw_application(
        raw_application
    )
...
def import_apptags(
    ...
    for raw_row in XlFileHelper.get_raw_cells_from_xl(
        excel_path, sheet_name, tag_column
    ):
    ...
    if not orderform_application_tags:
        message = (
            "No applications found in column %s (zero-based), exiting" % tag_column
        )
    ...
    for orderform_application_tag in orderform_application_tags:
        ...
        if application_obj.is_archived:
            ...
            logging.info(
                "%s is already active, no need to activate it", application_obj
            )

    all_active_apps_for_category = store.applications(
