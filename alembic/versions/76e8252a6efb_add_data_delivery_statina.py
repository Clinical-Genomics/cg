...
def upgrade():
    ...
    print(
        "setting family.data_delivery to statina where currently nipt-viewer", flush=True
    )
    bind = op.get_bind()
    bind.execute(
        "update family "
...
def downgrade():
    bind = op.get_bind()
    print(
        "setting family.data_delivery to nipt-viewer where currently statina", flush=True
    )
    bind.execute(
        "update family "
