...
def upgrade():
    ...
    op.add_column('sample_lane_sequencing_metrics', sa.Column('flow_cell_name', sa.String(length=128), nullable=False))
    op.create_foreign_key(None, 'sample_lane_sequencing_metrics', 'flowcell', ['flow_cell_name'], ['name'])
    op.add_column('sample_lane_sequencing_metrics', sa.Column('sample_internal_id', sa.String(length=128), nullable=False))
    op.create_foreign_key(None, 'sample_lane_sequencing_metrics', 'sample', ['sample_internal_id'], ['internal_id'])

    # Add relationships to Flowcell and Sample models
    op.add_column('flowcell', sa.Column('sample_lane_sequencing_metrics_id', sa.Integer(), nullable=True))
    op.create_foreign_key(None, 'flowcell', 'sample_lane_sequencing_metrics', ['sample_lane_sequencing_metrics_id'], ['id'])
    op.create_index(None, 'flowcell', ['sample_lane_sequencing_metrics_id'], unique=False)
    op.add_column('sample', sa.Column('sample_lane_sequencing_metrics_id', sa.Integer(), nullable=True))
    op.create_foreign_key(None, 'sample', 'sample_lane_sequencing_metrics', ['sample_lane_sequencing_metrics_id'], ['id'])
