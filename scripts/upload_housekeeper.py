...
def upload_cases(config, report_dir, case_dir):
    ...
    store_api = Store(config["database"])
    ...
    for case_id in fam_names:
        ...
        try:
            ...
            if analysis:
                print(
                    f"Analysis already stored in ClinicalDB: {case_id} : {analysis_start}, skipping upload to ClinicalDB"
                    f""
                )
            else:
...
def parse_deliverables_report(report_path) -> list:
    ...
    deliverables = dict((v[0], k.split("_")) for k, v in deliverables.items() if v is not None)
    bundle_files = []
